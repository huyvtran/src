...
public class cDBColumn
{
    ...
    #region bIdentity : Identity 여부.

    private bool _bIdentity;
    /// <summary>
    /// bIdentity : Identity 여부.
    /// </summary>
    public bool bIdentity
    {
    ...
    /// </summary>
	public cDBColumn()	{}
    public cDBColumn(string strName, string strType, int iLength, bool bUpdatable, bool bKey, bool bIdentity)
    {
    ...
    public string convertValue(string strValue)
    {
        switch (this.enType)
        {
            ...
            case SqlDbType.DateTime:
                {
                    return (string.IsNullOrEmpty(strValue) ? "GETDATE()" : (strValue.ToUpper().Equals("NULL") ? "null" : "'" + strValue.Replace("'", "''") + "'"));
                }
            default:
                {
    ...
    public NameValueCollection createQuery(typeQuery enType, string strValue)
    {
        ...
        switch (enType)
        {
            case typeQuery.INSERT:
                {
                    if (this.bUpdatable && !this.bIdentity)
                    {
