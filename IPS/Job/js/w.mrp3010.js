    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
				...
				//{ name: "추가", value: "추가" },
				{ name: "작성", value: "Check Sheet", icon: "실행" },
				{ name: "수정", value: "마감(취소)", icon: "실행" },
				{ name: "사양서", value: "사양서보기", icon: "기타" },
				{ name: "저장", value: "저장" },
				{ name: "닫기", value: "닫기" }
			]
        };
...
function processPopup(param) {
    ...
    if (param.element == "사양서") {
        ...
        var file_id = gw_com_api.getValue("grdData_현황", row, "file_id", true);
        if (file_id == "" || file_id == "undefined" || file_id == undefined) {
            gw_com_api.messageBox([{ text: "사양서가 등록되지 않았습니다." }]);
            return;
        }
