    ...
    UI: function () {
                        ...
                        element: [
                            {
                                ...
                                editable: { type: "text", size: 7, maxlength: 10, validate: { rule: "required", message: "계약기간" } }
                            },
			                {
			                    name: "ymd_to", label: { title: "~" }, mask: "date-ymd",
			                    editable: { type: "text", size: 7, maxlength: 10, validate: { rule: "required", message: "계약기간" } }
        ...
        var args = {
            ...
            element: [
                ...
                { header: "사업부", name: "dept_area_nm", width: 60, align: "center" },
        ...
        case "인증":
            {
                ...
                if (rtn > 0)
                {
                    ...
                    $.each(ids, function () {
                        doc_id += (doc_id == "" ? "" : ",") + gw_com_api.getValue("grdList_MAIN", this, "doc_id", true);
                        plain_txt += (plain_txt == "" ? "" : ",") + gw_com_api.getValue("grdList_MAIN", this, "doc_no", true);
                    });
        ...
        case "연장":
            {
                ...
                if (param.object == "lyrMenu") {
                    ...
                    if (gw_com_api.showMessage("선택된 계약목록의 계약기간을 " + gw_com_api.Mask(crs_date, "date-ymd") + " ~ " +
                        gw_com_api.Mask(cre_date, "date-ymd") + "로 변경합니다.\n계속하시겠습니까?", "yesno")) {
                        var row = [];
                        $.each(ids, function () {
                            var rowdata = gw_com_api.getRowData("grdList_MAIN", this);
                            row[row.length] = {
                                crud: "U",
                                column: [
                                    { name: "doc_id", value: rowdata["doc_id"] },
                                    { name: "crs_date", value: crs_date },
                                    { name: "cre_date", value: cre_date },
                                    { name: "fr_date", value: rowdata["fr_date"] == "" ? rowdata["crs_date"] : rowdata["fr_date"] },
                                    { name: "to_date", value: rowdata["to_date"] == "" ? rowdata["cre_date"] : rowdata["to_date"] }
                                ]
                            }
                        });
                        ...
                        var args = {
                            ...
                            param: [{ query: "ECM_1020_1", row: row }],
                            handler: {
