...
var gw_job_process = {
    ...
    UI: function () {
        ...
        //==== List Grid : 출고대상 품목정보
        var args = {
            targetid: "grdData_List", query: "SRM_4521_S_1", title: "반입가능 품목정보",
        ...
        //==== Item Form : 품목 정보
        var args = {
            targetid: "frmData_ITEM", query: "SRM_4521_S_1", type: "TABLE", title: "품목 정보",
...
function processRetrieveEnd(param) {
    ...
    if (iRowCount == 1) {
        ...
        rows.push({
            ...
            , track_no: gw_com_api.getValue("grdData_List", 1, "track_no", true)
            , io_unit: gw_com_api.getValue("grdData_List", 1, "unit", true)
            , inv_qty: gw_com_api.getValue("grdData_List", 1, "inv_qty", true)
            , dlv_date: gw_com_api.getValue("grdData_List", 1, "dlv_date", true)
        });
        ...
        if (gw_com_api.getValue("grdData_List", iRowCount, "inv_qty", true) > 0) {
            gw_com_api.messageBox([{ text: "재고가 있는 품목입니다." }], 300);
        } else if (gw_com_api.getValue("grdData_List", iRowCount, "label_tp", true) == "2") {
            var msg = gw_com_api.getValue("grdData_List", iRowCount, "issue_qty", true) +
                "[" + gw_com_api.getValue("grdData_List", iRowCount, "unit", true) + "]";
            gw_com_api.messageBox(
                [
                    { text: "통합 바코드 품목입니다." },
                    { text: "수량 " + msg + "(을)를 입고하시겠습니까?" }
                ], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", { rows: rows, env: true } );
        } else {
            processSend({ rows: rows, io_qty: 1 });
        }
    } else if (iRowCount < 1) {
...
function processSend(param) {
    ...
    param.rows[0].io_qty = param.io_qty;
                ...
                case gw_com_api.v_Message.msg_confirmBatch:
                    {
                        if (param.data.result == "YES") {
                            processSend({ rows: param.data.arg.rows, io_qty: gw_com_api.getValue("grdData_List", 1, "issue_qty", true) });
                        }
                    }
                    break;
            }
