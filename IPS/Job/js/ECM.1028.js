...
var gw_job_process = {
    ...
    ready: function () {
        ...
        var args = {
            request: [
				{
				    type: "PAGE", name: "ISCM81", query: "DDDW_CM_CODE",    // 사업부
    ...
    UI: function () {
                ...
                row: [
                    ...
                    },
                    {
                        element: [
                            ...
                            { name: "cstat", editable: { type: "hidden" }, hidden: true },
                            { name: "chk_emp", editable: { type: "hidden" }, hidden: true },
                            { name: "chk_date", editable: { type: "hidden" }, hidden: true },
                            { name: "chk_req_date", editable: { type: "hidden" }, hidden: true },
                            { name: "chk_rmk", editable: { type: "hidden" }, hidden: true },
        ...
        //=====================================================================================
        var args = {
            targetid: "grdData_DOC", query: "ECM_1028_3", title: "계약문서",
            height: "100%", show: true, caption: true, pager: false, selectable: true, number: true,
...
function processButton(param) {
    ...
    switch (param.element) {
        ...
        case "SEND_SUPP_CNL":
            {
                param.stat_type = param.element;
                gw_com_api.messageBox([{ text: "전송 취소처리 하시겠습니까?" }], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", param);
            }
            break;
        case "CHK_CNL":
            {
                param.stat_type = param.element;
                gw_com_api.messageBox([{ text: "검토 취소처리 하시겠습니까?" }], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", param);
            }
            break;
        default:
            {
...
function processSave(param) {
    ...
    // cert_yn = '0' => 계약완료
    if (gw_com_api.getValue("frmData_MAIN", 1, "cert_yn") == "0") {
        gw_com_api.setValue("frmData_MAIN", 1, "astat", "CFM_A");
        gw_com_api.setValue("frmData_MAIN", 1, "pstat", "CFM");
    }
...
function processBatch(param) {
    ...
    if (param.element == "APPR_A") {
        ...
        var proc = {
            ...
            input: [
                { name: "type", value: param.stat_type == undefined ? "U" : param.stat_type, type: "varchar" },
...
function setButton(param) {
    ...
    //=====================================================================================
    if (param.astat == "REG" || param.astat == "WAT_E" || param.astat == "CHK_A") {
    ...
    //            gw_com_api.hide("lyrMenu_B");
    //            gw_com_api.hide("lyrMenu_C");
    //            gw_com_api.hide("lyrMenu_D");
    //            gw_com_api.hide("lyrMenu_E");
    //            gw_com_api.hide("lyrMenu_F");
    //            gw_com_api.show("lyrMenu_SUB");
    //            gw_com_api.show("lyrMenu_DOC");
    //        }
    //        break;
