...
var gw_job_process = {
    ...
    UI: function () {
        ...
        //=====================================================================================
        var args = {
            targetid: "lyrInput", type: "FREE", title: "",
            trans: true, show: true, border: false, align: "left",
            editable: { bind: "open", focus: "plan_date" },
            content: {
                row: [
                    {
                        element: [
                            {
                                name: "plan_date", label: { title: "적용예정일 :" }, mask: "date-ymd",
    ...
    procedure: function () {
        ...
        if (v_global.process.param != "") {
            ...
            gw_com_api.setValue("lyrInput", 1, "plan_date", QUERY.getPlanDate({}));
...
function processSave(param) {
    ...
    var err = false;
    var plan_date = gw_com_api.getValue("lyrInput", 1, "plan_date");
    gw_com_api.setError(false, "lyrInpur", 1, "plan_date");
    if (plan_date == "") {
        err = true;
        gw_com_api.setError(true, "lyrInpur", 1, "plan_date");
    }
    if (err) {
        gw_com_api.messageBox([
                { text: "적용예정일을 입력하세요." }
    ...
    processBatch({ nomessage: false, plan_date: plan_date });
...
function processBatch(param) {
    ...
    var args = {
        url: "COM",
        procedure: "PROC_ECO_SUPP", nomessage: param.nomessage,
        input: [
            { name: "eco_no", value: v_global.logic.key, type: "varchar" },
            { name: "user_id", value: gw_com_module.v_Session.USR_ID, type: "varchar" },
            { name: "plan_date", value: param.plan_date == undefined ? "" : param.plan_date, type: "varchar" }
        ],
        handler: {
    ...
    getPlanDate: function (param) {
        ...
        var args = {
            ...
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    ...
                    "&QRY_COLS=plan_date" +
                    "&CRUD=R" +
                    "&arg_eco_no=" + v_global.logic.key + "&arg_user_id=" + gw_com_module.v_Session.USR_ID,
            handler_success: successRequest
        };
        //=================== async : false ========================
        $.ajaxSetup({ async: false });
