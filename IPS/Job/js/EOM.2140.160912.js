...
var gw_job_process = {
    ...
    UI: function () {
            ...
            content: { row: [
                    { element: [
                        ...
                        { name: "eq_cd", label: { title: "호기 :" },
                            editable: { type: "select", data: { memory: "연구설비", unshift: [{ title: "-", value: "%"}] } 
        ...
        //==== Grid : Main ====
        var args = { targetid: "grdData_Main", query: "EOM_2140_M_1", title: "설비 상태 이력",
            ...
            element: [
				...
				{ header: "현상분류(대)", name: "status_tp1", width: 80, align: "center" 
					, format: { type: "select", data: { memory: "현상분류1" } } },
                { header: "실적일자", name: "fr_dt", width: 80, align: "center" },
                { header: "실적시간(hr)", name: "addtime", width: 80, align: "center" },
                { header: "Wafer 사용매수", name: "wafer", width: 60, align: "center", mask: "numeric-int" },
        ...
        //==== Form : Main ====
        var args = { targetid: "frmData_Sub", query: "EOM_2140_S_1", type: "TABLE", title: "등록 내역",
            ...
            content: { width: { label: 80, field: 150 }, height: 25,
                row: [
                    { element: [
                        ...
                        { name: "addtime", align: "center", editable: { type: "text", width: 155 }, mask: "numeric-float1" },
                        
                        { name: "to_date", mask: "date-ymd", align: "center", hidden: true },
                        { name: "to_time", mask: "time-hm", align: "center", hidden: true }
	                    ]
                    },
                    { element: [
                        { header: true, value: "Module", format: { type: "label"} },
                        { name: "eq_module1", format: { width: 140 },
                            style: { colspan: 3, colfloat: "float" },
                            editable: { type: "select", width: 140, data: { memory: "설비모듈", key: ["eq_cd"] }, validate: { rule: "required" } }
                        },
                        //{
                        //    name: "eq_module2", format: { width: 90 }, hidden: true,
                        //    style: { colfloat: "floating" },
                        //    editable: { type: "select", width: 90, data: { memory: "설비모듈", key: ["eq_cd"] } }
                        //},
                        //{
                        //    name: "eq_module3", format: { width: 90 }, hidden: true,
                        //    style: { colfloat: "floating" },
                        //    editable: { type: "select", width: 90, data: { memory: "설비모듈", key: ["eq_cd"] } }
                        //},
                        //{
                        //    name: "eq_module4", format: { width: 90 }, hidden: true,
                        //    style: { colfloat: "floated" },
                        //    editable: { type: "select", width: 90, data: { memory: "설비모듈", key: ["eq_cd"] } }
                        //},
                        { header: true, value: "현상분류", format: { type: "label"} },
                        { name: "status_tp1" , format: { width: 90 }, style: { colspan: 3, colfloat: "float" },
							editable: { type: "select", width: 90, validate: { rule: "required"} 
								, data: { memory: "현상분류1" }
								, change: [ { name: "status_tp2", memory: "현상분류2", key: [ "status_tp1" ] },
                        ...
                        { name: "status_tp2" , format: { width: 110 }, style: { colfloat: "floating" }, 
							editable: { type: "select", width: 110
								, data: { memory: "현상분류2", key: [ "status_tp1" ] }
								, change: [ { name: "status_tp3", memory: "현상분류3", key: [ "status_tp2" ] },
											{ name: "status_tp4", memory: "현상분류4", key: [ "status_tp3" ] } ] 
								} 
                        },
                        ...
                        { name: "status_tp4" , format: { width: 115 }, style: { colfloat: "floated" },
							editable: { type: "select", width: 115 
								, data: { memory: "현상분류4", key: [ "status_tp3" ] }
								} 
                        }
	                    ]
                    },
                    { element: [
                        ...
                        { name: "issue_rmk", style: { colspan: 5 },
                            format: { type: "text", width: 730 },
                            editable: { type: "text", width: 730}
                        },
                        { header: true, value: "Wafer 사용매수", format: { type: "label"} },
                        { name: "wafer", mask: "numeric-int",
		                    format: { type: "text", width: 160 },
                    ...
                    { element: [
                        ...
                        { name: "unit_rmk", style: { colspan: 7 },
                            format: { type: "textarea", rows: 7, width: 1015 },
                            editable: { type: "textarea", rows: 7, width: 1015}
                        },
                        ...
                        { name: "get_to_time", hidden: true, editable: { type: "hidden" } },
                        { name: "get_to_date", hidden: true, editable: { type: "hidden" } },
                        { hidden: true, name: "unit_user", editable: { type: "text" } },
                        { hidden: true, name: "data_key", editable: { type: "hidden" } },
                        { name: "ymd_to", hidden: true, editable: { type: "text", width: 80 } },
                        { name: "ymd_fr", hidden: true, editable: { type: "text", width: 40 } }
	                    ]
                    }
                ]
            }
...
function processItemchanged(param) {
    ...
    var fr_time = gw_com_api.getValue("frmData_Sub", 1, "fr_time");
    var addtime = gw_com_api.getValue("frmData_Sub", 1, "addtime");
    ...
    if (param.object == "frmData_Sub") {
        switch (param.element) {
            ...
            case "fr_time":
            case "addtime":
                gw_com_api.setValue("frmData_Sub", 1, "to_date", todt, false);
                gw_com_api.setValue("frmData_Sub", 1, "to_time", totm, false);
        }
    }
}
//----------
		...
		args = {
			...
			source: {
				...
				element: [
                    ...
                    { name: "addtime", argument: "arg_addtime" },
                    { name: "ymd_fr", argument: "arg_ymd_fr" },
                    { name: "ymd_to", argument: "arg_ymd_to" }
				]
			},
