...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = { targetid: "frmData_Top", query: "EHM_2311_M_2", type: "TABLE", title: "A/S 발생 정보",
            ...
            content: { width: { label: 100, field: 180 }, height: 25,
                row: [
                    { element: [
            ...
            content: { width: { label: 94, field: 180 }, height: 25,
                row: [
                    { element: [
                        ...
                        { name: "rqst_tp"
							, format: { type: "select", data: { memory: "발생구분" }, validate: { rule: "required"} } },
                    ...
                    { element: [
                        ...
                        { name: "part_tp1"
							, format: { type: "select", data: { memory: "부위구분" }, validate: { rule: "required"} } },
                        { header: true, value: "PM상세구분", format: { type: "label"} },
                        { name: "part_tp2"
							, format: { type: "select", data: { memory: "PM상세구분" }, validate: { rule: "required"} } },
                        { header: true, value: "초발/재발", format: { type: "label"} },
                        { name: "issue_cnt"
							, editable: { type: "text" } },
                        { header: true, value: "진행상태", format: { type: "label"} },
                        { name: "pstat"
							, format: { type: "select", data: { memory: "진행상태" }, validate: { rule: "required"} } }
	                    ]
                    },
                    { element: [
                        { header: true, value: "현장조치결과", format: { type: "label"} },
                        { name: "result_tp"
							, format: { type: "select", data: { memory: "현장조치결과" }, validate: { rule: "required"} } },
                        { header: true, value: "Item구분", format: { type: "label"} },
                        { name: "item_tp"
							, format: { type: "select", data: { memory: "Item구분" }, validate: { rule: "required"} } },
                    ...
                    { element: [
                        ...
                        { name: "standard_yn"
							, format: { type: "select", data: { memory: "유무" }, validate: { rule: "required"} } },
                        { header: true, value: "이행유무", format: { type: "label"} },
                        { name: "actual_yn"
							, format: { type: "select", data: { memory: "이행유무" }, validate: { rule: "required"} } },
                    ...
                    { element: [
                        ...
                        { name: "reason_tp"
							, format: { type: "select", data: { memory: "원인추정" }, validate: { rule: "required"} } },
                        { header: true, value: "교육체계", format: { type: "label"} },
                        { name: "edu_yn"
							, format: { type: "select", data: { memory: "유무" }, validate: { rule: "required"} } },
                        { header: true, value: "준수여부", format: { type: "label"} },
                        { name: "rule_tp"
							, format: { type: "select", data: { memory: "준수여부" }, validate: { rule: "required"} } },
        ...
        var args = { targetid: "grdData_Sub", query: "EHM_2312_S_1", title: "담당 부서",
            ...
            element: [
				...
				{ header: "재발방지", name: "act_tp1", width: 120, align: "left"
				    , format: { type: "select", data: { memory: "재발방지" }, validate: { rule: "required"} } },
				{ header: "대책구분", name: "act_tp2", width: 120, align: "left"
				    , format: { type: "select", data: { memory: "대책구분", key: ["act_tp1"] }, validate: { rule: "required"} } },
            ...
            content: { width: { label: 94, field: 180 }, height: 25,
                row: [
                    ...
                    { element: [
                        ...
                        { name: "rslt_user_nm", mask: "search", display: true, editable: { type: "text"} },
...
function processSend(param) {
    ...
    gw_com_api.messageBox([
        ...
        { text: "계속 하시겠습니까?" }
    ], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", { type: "고분보-RQST" });

}
//---------- Batch : 고분보 발행 통보 처리 Procedure 실행 (PROC_MAIL_QDM_고분보)
function processBatch(param) {
    var args = {
        url: (param.type == "고분보-RQST") ? "COM" : gw_com_module.v_Current.window + ".aspx/" + "Mail",
        procedure: "PROC_MAIL_QDM_고분보", nomessage: true,
        argument: [
