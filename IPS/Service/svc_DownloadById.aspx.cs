...
public partial class cFileDownload : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                string strSQL = "SELECT file_id, file_nm, REPLACE(file_path, '\\', '\\\\') AS file_path, file_ext FROM ZFILE WHERE file_id = @file_id";
                ...
                {
                    ClientScript.RegisterStartupScript(GetType(), "close", "alert(\"요청한 파일을 찾을 수 없습니다.\\n" + strFilePath + "\");window.close();", true);
                }
            }
            catch
            {
