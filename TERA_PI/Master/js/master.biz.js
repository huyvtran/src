...
//----------
var v_openMenu = (gw_com_api.getPageParameter("MENU_ID") == "" ? undefined : { menu_id: gw_com_api.getPageParameter("MENU_ID") });
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// process.
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
var gw_job_process = {
    ...
    ready: function () {
        ...
        function successSession(data) {
            ...
            var qry_id = "PLM_MENU";
            // for APS
            if (v_Session.USER_TP == "EMP" && location.hostname.split(".")[0].toUpperCase() == "PI" && location.hostname.split(".")[1].toUpperCase() == "APSYSTEMS") {
        ...
        function successMenu(data) {
            ...
            $.each(data, function (i) {
                if (this.DATA[column.level_no] == 1) {
                    ...
                    var args = this.DATA[column.menu_args];
                    ...
                    if (this.DATA[column.child_cnt] == "0") {
                        ...
                        if (menu_id == "POP_9080") {
                            $("#menu_" + menu_id).click(function () {
                                var url = "/" + object + "?menu_id=" + menu_id + "&user_id=" + v_Session.USR_ID;
                                window.open(url, "", "");
                            });
                        }
                        // KPI 관리자용 C/S PROGRAM
                        else if (menu_id.substring(0, 5) == "KPI10") {
                            $("#menu_" + menu_id).click(function () {
                                var url = "/" + object + "?menu_id=" + menu_id + "&user_id=" + v_Session.USR_ID + (args == "" ? "" : "&" + args);
                                window.open(url, "", "");
                            });
                        } else {
                            if (v_openMenu != undefined) {
                                if (v_openMenu.menu_id == menu_id) {
                                    v_openMenu.object = object;
                                    v_openMenu.title = title;
                                    v_openMenu.url = url;
                                    v_openMenu.args = args;
                                    v_openMenu.menu_id = menu_id;
                                }
                            }
                            $("#menu_" + menu_id).click(function () {
                                gw_job_process.processTab(object, title, url, args, menu_id);
                            });
                        }
                    }
                }
            ...
            if (v_openMenu != undefined) {
                if (v_openMenu.object == undefined || v_openMenu.object == "")
                    gw_com_api.showMessage("권한이 없습니다.");
                else
                    gw_job_process.processTab(v_openMenu.object, v_openMenu.title, v_openMenu.url, v_openMenu.args, v_openMenu.menu_id);
            }
        ...
        function click_imgNotice() {
            ...
            gw_job_process.processTab("w_sys2034", "공지사항 조회", "../job/w_sys2034.aspx", "", "");
    ...
    processTab: function (index, title, content, args, menu_id) {
        ...
        // KPI지표 상세 화면 관련 예외처리
        if (menu_id.substring(0, 9) == "KPI_1111_")
            index = menu_id;
