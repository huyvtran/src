...
var gw_com_api = {
    ...
    getPageParameter: function (name) {
        ...
        else return decodeURIComponent(results[1].replace(/\+/g, " ").replace(/\=/g, "&#61").replace(/\;/g, "&#59").replace(/\:/g, "&#58")
                                                 .replace(/\?/g, "&#63").replace(/\//g, "&#47").replace(/\%/g, "#%37"));
    ...
    // filter select box.
    // #region
    filterSelect: function (id, row, name, rule, grid) {
    ...
    getFindRow: function (id, col, val) {
        ...
        return row;
		//var nRowCnt = this.getRowCount(id);
    ...
    getColCount: function (id) {
        ...
        return $("#" + id + "_data").getGridParam('colModel');
