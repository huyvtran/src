...
(function($, document) {
	$.fn.AjaxFlagMenu = function(Options) {
		...
		function AjaxFlagMenuItem(_Title, _onOutIcon, _onOverIcon, _onClickIcon, _HtmlStatusContent, _url, _data) {
			...
			var Icon = $("<tr><td align='left' style='padding-left:20px; width:15px;'><img style='width:8px; height:8px;'/></td><td align='left'>" + _Title + "</td></tr>");
			...
			_AjaxFlagMenuItem.attr("title", _Title);
			...
			_AjaxFlagMenuItem.click(function() {
				...
				gw_job_process.processTab(_data, _Title, _url, "");
				alert('a');
				window.scroll(0, 0);
			});
		};
		...
		var iTems = new Array();
		var AjaxFlagMenu = $("<ul style='margin:0; padding:0;'></ul>");
