...
var gw_job_process = {
    ...
    ready: function () {
        ...
        function start() {
            ...
            var id = gw_com_api.getPageParameter("User");
            var args = {
                request: "PAGE",
...
function successSession(data) {
    ...
    gw_com_module.v_Current.window = "w_eccb4010_if";
    gw_com_module.v_Current.launch = "CHILD";
...
function processInsert(param) {
    ...
    if (param.sub) {
        ...
        v_global.logic.if_key = gw_com_api.getPageParameter("Key");
        if (v_global.logic.if_key == "") {
            gw_com_api.showMessage("인터페이스 규칙이 적합하지 않습니다.\n관리자에게 문의 바랍니다.");
            window.close();
        }
        var args = {
            type: "PAGE",
...
function processSave(param) {
    ...
    if (err) {
        ...
        //]);
        gw_com_api.showMessage("[기타]에 대한 내용이 입력되지 않았습니다.");
        return false;
    }
...
function processApprove(param) {
    ...
    if (status != '없슴' && status != '미처리' && status != '반송' && status != '회수') {
        ...
        //], 420);
        gw_com_api.showMessage("결재 " + status + " 자료이므로 처리할 수 없습니다.");
        return false;
    }
