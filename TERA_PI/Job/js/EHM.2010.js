    ...
    UI: function () { 
        ...
        var args = {
            ...
            memory: "교체구분"
				...
				{ header: "제조사", name: "apart_supp", width: 150, align: "left", hidden: true },
				{ header: "모델", name: "apart_model", width: 150, align: "center", hidden: true },
				{ header: "규격(REV)", name: "apart_rev", width: 150, align: "center", hidden: true },
				{ header: "비고(REV)", name: "apart_rmk", width: 300, align: "left", hidden: true },
				{
				...
				{ header: "제조사", name: "bpart_supp", width: 150, align: "left", hidden: true },
				{ header: "모델", name: "bpart_model", width: 150, align: "center", hidden: true },
				{ header: "규격(REV)", name: "bpart_rev", width: 150, align: "center", hidden: true },
				{ header: "비고(REV)", name: "bpart_rmk", width: 300, align: "left", hidden: true },
				{
    ...
    procedure: function () {
        ...
        function itemchanged_grdData_교체PART(ui) {
            ...
            switch (ui.element) {
                ...
                case "apart_cd":
                case "bpart_cd":
                    var item_info = (ui.value.current == "" ? undefined : Query.getItemInfo({ item_cd: ui.value.current }));
                    var element = ui.element.substr(0, ui.element.length - 2) + "nm";
                    //gw_com_api.setValue(ui.object, ui.row, ui.element, item_info[0], true, false, false);
                    gw_com_api.setValue(ui.object, ui.row, element, (item_info == undefined ? "" : item_info[1]), true, false, false);
                    break;
                case "apart_nm":
                case "bpart_nm":
                    var item_info = (ui.value.current == "" ? undefined : Query.getItemInfo({ item_nm: ui.value.current }));
                    var element = ui.element.substr(0, ui.element.length - 2) + "cd";
                    //gw_com_api.setValue(ui.object, ui.row, ui.element, item_info[0], true, false, false);
                    gw_com_api.setValue(ui.object, ui.row, element, (item_info == undefined ? "" : item_info[1]), true, false, false);
                    break;
            }
...
function processExport() {
    ...
    var args = {
        ...
        option: [
            { name: "PRINT", value: "xls" }, { name: "PAGE", value: gw_com_module.v_Current.window }
        ],
        target: { type: "FILE", id: "lyrDown", name: "Service 작업 결과 보고서" },
        handler: {
...
//----------
var Query = {
    getItemInfo: function (param) {
        var rtn;
        var args = {
            request: "PAGE",
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    "?QRY_ID=dddw_item" +
                    "&QRY_COLS=item_cd,item_nm,spec" +
                    "&CRUD=R" +
                    "&arg_item_cd=" + (param.item_cd == undefined ? "" : encodeURI(param.item_cd)) +
                    "&arg_item_nm=" + (param.item_nm == undefined ? "" : encodeURI(param.item_nm)),
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        ...
        case gw_com_api.v_Stream.msg_uploaded_ASISSUE: {
                var args = {
                    source: { type: "GRID", id: "grdData_발생정보", row: "selected",
                        element: [
				            { name: "issue_no", argument: "arg_issue_no" }
			            ]
                    },
