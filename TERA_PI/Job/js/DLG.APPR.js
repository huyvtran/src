...
function processButtonStat(param) {
    ...
    if (param.reset) {
        ...
        if ($.inArray(v_global.logic.appr_yn, ["", "대기"]) == -1 &&
            $.inArray(v_global.logic.appr_yn_next, ["대기", undefined]) >= 0 &&
...
function createDW(param) {
    ...
    //----------
    if (param.editable) {
        args.content.row[0].element[1].format = { type: "textarea", rows: 2, width: 490 };
        args.content.row[0].element[1].editable = { type: "textarea", rows: 2, width: 490 };
    } else {
        args.content.row[0].element[1].format = { type: "textarea", rows: 2, width: 490 };
    }
    //----------
