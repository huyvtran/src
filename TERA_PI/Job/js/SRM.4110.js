    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
				...
				{ name: "출력", value: "납품서(입고증)", icon: "출력" },
				//{ name: "라벨", value: "현품표", icon: "기타" },
				{ name: "라벨2", value: "현품표", icon: "기타" },
				{ name: "닫기", value: "닫기" }
            ]
        };
        //----------
        ...
        var args = {
            ...
            element: [
				...
				{ header: "납품(예정)일자", name: "dlv_date", width: 70, align: "center", mask: "date-ymd" },
				{ header: "시간", name: "dlv_time", width: 50, align: "center", mask: "time-hm" },
				{ header: "검수(희망)일자", name: "qc_date", width: 70, align: "center", mask: "date-ymd" },
				{ header: "품목수", name: "item_cnt", width: 50, align: "center" },
				{ header: "납품담당자", name: "dlv_user", width: 60, align: "center" },
				{ header: "진행상태", name: "pstat", width: 60, align: "center", hidden: true },
				{ header: "검수자", name: "chk_user_nm", width: 60, align: "center" },
				{ header: "검수일자", name: "chk_date", width: 70, align: "center", mask: "date-ymd" },
				{ header: "인수자", name: "acp_user", width: 60, align: "center", hidden: true },
				{ header: "인계일시", name: "acp_dt", width: 70, align: "center", mask: "date-ymd", hidden: true },
        ...
        var args = {
            ...
            caption: true, height: "100%", pager: true, show: true, number: true, multi: true, checkrow: true, //key: true,
            element: [
				...
				{ header: "발주수량", name: "pur_qty", width: 60, align: "right" },
				{ header: "단위", name: "pur_unit", width: 40, align: "center" },
				{ header: "납품수량", name: "dlv_qty", width: 60, align: "right" },
				{ header: "라벨", name: "label_tp", width: 40, align: "center", hidden: true },
				{
				...
				{ header: "바코드", name: "barcode", width: 90, align: "center", hidden: true },
...
function processExport(param) {
    ...
    if (param.element == "출력") {
        ...
        if (row < 1) {
            gw_com_api.messageBox([{ text: "출력 대상을 선택하세요." }]);
            return;
        }
        if (v_global.logic.Supp && gw_com_api.getValue("grdData_Main", row, "astat", true) == "") {
            // 성적서 체크
            if (Query.chkPrintable({ dlv_no: key }) == "0") {
                gw_com_api.messageBox([{ text: "성적서가 등록되지 않은 품목이 있습니다." }, { text: "성적서를 확인하시기 바랍니다." }],
                    420, gw_com_api.v_Message.msg_alert, "ALERT", { handler: processOpenMenu, param: { obj_id: "SRM_6110", title: "수입성적서 등록" } });
                return false;
            }

            // 거래명세서 필수 체크
            if (gw_com_api.getFindRow("grdData_Sub", "direct_yn", "1") > 0) {
                if (gw_com_api.getRowCount("grdData_File1") < 1) {
                    gw_com_api.messageBox([{ text: "선입고 품목이 있을 경우 거래명서를 꼭 첨부해야 합니다." }], 450);
                    return false;
                }
            }
            v_global.process.handler = processExport2;
            gw_com_api.messageBox([
                { text: "입고증 출력 후 데이터 수정이 불가합니다." },
                { text: "계속하시겠습니까?." }
            ], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", { handler: processExport2, param: param });
        } else {
            processExport2(param);
        }
        return;
    }
    else if (param.element == "라벨" || param.element == "라벨2") {
        ...
        // TERA
        var ext = (gw_com_module.v_Session.USER_TP == "EMP" ? "A" : "B");
        var dlv_no = gw_com_api.getValue("grdData_Main", "selected", "dlv_no", true);
        var url = "/Report/App/SRM_4110_" + ext + "/SRM_DLV_RPT01_" + ext + ".application?menu_id=TeraDlvLabel&dlv_no=" + dlv_no + "&dlv_seq=" + SelRows;
        window.open(url, "", "scrollbars=yes,toolbar=yes,resizable=yes,width=100,height=100,left=0,top=0");
        //var url = "/Report/App/SRM_4110_" + ext + "/SRM_DLV_RPT01_" + ext + ".application";
        //var data = {
        //    menu_id: "TeraDlvLabel",
        //    dlv_no: dlv_no,
        //    dlv_seq: SelRows
        //};
        //var option = "scrollbars=yes,toolbar=yes,resizable=yes,width=100,height=100,left=0,top=0";
        //gw_com_api.postOpen(url, data, "TeraDlvLabel", option);
        return;
...
*/
        ...
        option: [
            { name: "PRINT", value: "PDF" /*(param.element == "라벨2" ? "XLS" : "PDF")*/ },
...
//----------
function processExport2(param) {

    var ext = (gw_com_module.v_Session.USER_TP == "EMP" ? "A" : "B");
    var dlv_no = gw_com_api.getValue("grdData_Main", "selected", "dlv_no", true);
    var url = "/Report/App/SRM_4110_" + ext + "/SRM_DLV_RPT01_" + ext + ".application?menu_id=TeraDlvReport&dlv_no=" + dlv_no;
    window.open(url, "", "scrollbars=yes,toolbar=yes,resizable=yes,width=100,height=100,left=0,top=0");

    if (v_global.logic.Supp && gw_com_api.getValue("grdData_Main", "selected", "astat", true) == "") {
        var row1 = [{
            crud: "U",
            column: [
                { name: "dlv_no", value: dlv_no },
                { name: "astat", value: "입고증" },
                { name: "astat_dt", value: gw_com_api.getDate() }
            ]
        }];

        var row2 = new Array();
        $.each(gw_com_api.getRowIDs("grdData_Sub"), function () {
            row2.push({
                crud: "U",
                column: [
                    { name: "dlv_no", value: dlv_no },
                    { name: "astat", value: "입고증" },
                    { name: "astat_dt", value: gw_com_api.getDate() },
                    { name: "dlv_seq", value: gw_com_api.getValue("grdData_Sub", this, "dlv_seq", true) }
                ]
            });
        });
        ...
        var args = {
            ...
            nomessage: true,
            param: [
                { query: "SRM_4120_M_1", row: row1 },
                { query: "SRM_4120_S_1", row: row2 }
            ],
            handler: {
                success: successSave,
                param: param
        ...
        case "supp_nm":
            {
                ...
                v_global.logic.search = {
                    supp_cd: (ui.element == "supp_cd" ? gw_com_api.getValue(ui.object, ui.row, ui.element) : ""),
                    supp_nm: (ui.element == "supp_nm" ? gw_com_api.getValue(ui.object, ui.row, ui.element) : "")
                };
                var args = {
...
//----------
function processOpenMenu(param) {

    var args = {
        ID: gw_com_api.v_Stream.msg_linkPage,
        to: { type: "MAIN" },
        data: {
            page: param.obj_id, title: param.title
        }
    };
    gw_com_module.streamInterface(args);
        ...
        case gw_com_api.v_Stream.msg_resultMessage: {
            ...
            switch (param.data.ID) {
                case gw_com_api.v_Message.msg_alert:
                    { if (param.data) param.data.arg.handler(param.data.arg.param); } break;
                case gw_com_api.v_Message.msg_confirmSave:
                    { if (param.data.result == "YES") processDelete(param.data.arg); } break;
                case gw_com_api.v_Message.msg_confirmRemove:
                    { if (param.data.result == "YES") processRemove(param.data.arg); } break;
                case gw_com_api.v_Message.msg_confirmBatch:
                    { if (param.data.result == "YES") param.data.arg.handler(param.data.arg); } break;
