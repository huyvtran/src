...
var gw_job_process = {
    ...
    procedure: function () {
        ...
        function click_lyrMenu_2_추가(ui) {
            ...
            var args = {
                ...
                data: [
                    ...
                    { name: "plan_qty", value: gw_com_api.getValue("grdData_현황", "selected", "delay_qty", true) }
                ]
            };
        ...
        function click_lyrMenu_2_삭제(ui) {
            ...
            if (!(gw_com_api.getCRUD("grdData_상세", "selected", true) == "create" ||
                gw_com_api.getCRUD("grdData_상세", "selected", true) == "initialize")) {
                gw_com_api.messageBox([
                    { text: "이미 저장된 내용은 삭제할 수 없습니다." },
                    { text: "구분을 [취소]로 변경, 저장 후 새로 등록하세요." }
                ], 420);
                return;
            }
            if (gw_com_api.getValue("grdData_현황", "selected", "cancel_yn", true) == "1") {
                        ...
                        if (ui.value.current == "0") {
                            ...
                            gw_com_api.setValue(
                                ...
                                (gw_com_api.getSelectedRow("grdData_현황") != null)
                                    ? gw_com_api.getValue("grdData_현황", "selected", "req_date", true) : ""/*,
                                ""*/
                            );
                        }
                        else {
                ...
                case "plan_date":
                    {
                        if (gw_com_api.unMask(ui.value.current, "date-ymd")
                            > gw_com_api.unMask(gw_com_api.getValue("grdData_현황", "selected", "req_date", true), "date-ymd"))
...
function checkBatchable(param) {
    ...
    gw_com_api.messageBox([
            ...
            { text: "◈ 납품가능일 : " + ((ymd_yn == "1") ? gw_com_api.Mask(ymd, "date-ymd") : "납기요구일") + "<br><br>", align: "left", margin: 30 },
            { text: "일괄 생성 하시겠습니까?" }
...
function processSave(param) {
    ...
    if (sum > gw_com_api.getValue("grdData_현황", "selected", "pur_qty", true)) {
        gw_com_api.messageBox([
            ...
            { text: "[수량] 합계는 [발주수량]보다 작거나 같아야 합니다." }
        ]);
        return false;
    }
...
function successSave(response, param) {
    ...
    //납기일 초과입력 알림
    //processBatch2({ pur_no: response[0].KEY[0].VALUE });
