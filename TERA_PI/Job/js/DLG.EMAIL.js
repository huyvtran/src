                ...
                row: [
                    {
                        element: [
                          ...
                          { name: "receiver", editable: { type: "hidden", width: 870 }, mask: "search" },
                          { name: "receive_user_id", hidden: true}
                        ]
                    },
                    {
                        element: [
                          { header: true, value: "참&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;조", format: { type: "label" } },
                          { name: "copy_receiver", editable: { type: "hidden", width: 870 }, mask: "search" },
                          { name: "copy_receive_user_id", hidden: true }
                        ]
                    },
                    {
                        element: [
                          { header: true, value: "제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목", format: { type: "label" } },
                          { name: "subject", editable: { type: "text", width: 870 } }
                        ]
                    },
                    {
                        control: true,
...
function processInit(param) {
    ...
    if (v_global.data.args.temp_id != undefined) {
        ...
        var args = {
            source: {
                ...
                argument: [
                    ...
                    { name: "arg_receiver", value: getReceiver({ type: "to" }) },
                    { name: "arg_copy_receiver", value: getReceiver({ type: "cc" }) }
                ]
            },
            target: [
        ...
        var args = {
            targetid: "frmInput",
            data: [
                { name: "receiver", value: getReceiver({ type: "to" }) },
                { name: "copy_receiver", value: getReceiver({ type: "cc" }) },
...
function processItemdblclick(param) {
    ...
    if (param.object == "frmInput") {
        if (param.element == "receiver" || param.element == "copy_receiver") {
            //var title = (param.element == "receiver" ? "받는사람 선택" : "참조자 선택");
            var title = "받는사람(참조자) 선택";
            v_global.event.data = {
                type: (param.element == "receiver" ? "to" : "cc"),
                list: (param.element == "receiver" ? v_global.data.args.to : v_global.data.args.cc)
            };

            var args = {
                type: "PAGE", page: "DLG_EMAIL_RECEIVER", title: title,
...
function processSend(param) {
    ...
    if (v_global.data.args.to == undefined || v_global.data.args.to == "") {
        alert("[받는사람]을 지정하세요.");
        return;
    }
    v_global.data.args.nomessage = true;    // div 팝업을 사용할 수 없네..
    ...
    v_global.data.args.stretch = false;
...
//----------
function getReceiver(param) {

    if (param == undefined || param.type == undefined) return "";
    var receiver = "";
    if (param.type == "to") {
        ...
        if (v_global.data.args.cc == undefined) return "";
        $.each(v_global.data.args.cc, function () {
            receiver += (receiver == "" ? "" : ", ") + (this.name == "" ? this.value : this.name);
        });
    }
    return receiver;
        ...
        case gw_com_api.v_Stream.msg_closeDialogue:
            {
                switch (param.from.page) {
                    case "DLG_EMAIL_RECEIVER":
                        if (param.data.type == "to") {
                            v_global.data.args.to = param.data.list;
                        } else {
                            v_global.data.args.cc = param.data.list;
                        }
                        gw_com_api.setValue(v_global.event.object, v_global.event.row, v_global.event.element, getReceiver({ type: param.data.type }));
                        break;
                }
