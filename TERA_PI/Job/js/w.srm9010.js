...
var gw_job_process = {
    ...
    UI: function () {
        ...
        //=====================================================================================
        var args = {
            targetid: "frmData_PASSWD", type: "FREE",
        ...
        var args = {
            ...
            content: {
                width: { label: 90, field: 232 }, height: 25,
                row: [
                    {
                        element: [
                            ...
                            { name: "rgst_no", mask: "biz-no", editable: { type: "hidden" } }
                        ]
                    },
                    {
                        element: [
                            { header: true, value: "대표전화", format: { type: "label" } },
                            { name: "tel_no", editable: { type: "hidden" } },
                            { header: true, value: "대표팩스", format: { type: "label" } },
                            { name: "fax_no", editable: { type: "hidden" } }
                        ]
                    },
                    {
                        element: [
                            { header: true, value: "우편번호", format: { type: "label" } },
                            { style: { colspan: 3 }, name: "zip_no", editable: { type: "hidden" } }
                        ]
                    },
                    {
                        element: [
                            { header: true, value: "회사주소", format: { type: "label" } },
                            { name: "addr1", editable: { type: "hidden" } },
                            { header: true, value: "회사주소 상세", format: { type: "label" } },
                            { name: "addr2", editable: { type: "hidden" } }
                        ]
                    }
                ]
            }
        ...
        var args = {
            ...
            caption: true, height: 96, pager: false, show: true, selectable: true, number: true,
            ...
            element: [
				...
				//{
				//    header: "사업부", name: "area_cd", width: 80, align: "center",
				//    format: { type: "select", data: { memory: "사업부", unshift: [{ title: "-", value: "-" }, { title: "공통", value: "CM" }] } },
				//    editable: { type: "select", data: { memory: "사업부", unshift: [{ title: "-", value: "-" }, { title: "공통", value: "CM" }] } }
				//},
                {
                    header: "발주서수신", name: "mail01_yn", width: 80, align: "center",
                    format: { type: "checkbox", title: "", value: "1", offval: "0" },
                    editable: { type: "checkbox", title: "", value: "1", offval: "0" }
                },
                {
                    header: "NCR수신", name: "mail02_yn", width: 80, align: "center",
                    format: { type: "checkbox", title: "", value: "1", offval: "0" },
                    editable: { type: "checkbox", title: "", value: "1", offval: "0" }
                },
				{
				    header: "부서", name: "dept_nm", width: 120, align: "center",
				    editable: { type: "hidden", maxlength: 15 }
				},
				{
				...
				},
				{
				    ...
				    editable: { type: "hidden", maxlength: 20 }
				},
				{
        ...
        $("#lyrNotice_notice").css({ "font-weight": "bold" });
        //=====================================================================================
        var args = {
            target: [
    ...
    procedure: function () {
        ...
        var args = {
            targetid: "lyrNotice",
            row: [
                { name: "notice", value: "상세 정보 및 담당자 정보 변경은 테라세미콘 구매팀 담당자에게 연락 후 수정할 수 있습니다." }
            ]
        };
...
function processLink() {
    ...
    var args = {
        source: {
            type: "GRID", id: "grdData_협력사", row: "selected",
            element: [
...
function processChangePasswd(param) {
    ...
    var current_passwd = hex_md5(gw_com_api.getValue("frmData_PASSWD", 1, "current_passwd"));
    ...
    if (current_passwd == hex_md5(new_passwd) || new_passwd.length < 6) {
        gw_com_api.messageBox([{ text: "사용 불가능한 비밀번호입니다." }]);
        return;
    }
    gw_com_api.setValue("grdData_협력사", "selected", "user_pw", hex_md5(confirm_passwd), true);
    var qryZfile = {
        query: "w_srm9010_M_1",
        row: [{
            crud: "U",
            column: [
                ...
                { name: "user_pw", value: hex_md5(confirm_passwd) }
            ]
        }]
    };
        ...
        case gw_com_api.v_Stream.msg_openedDialogue:
            {
                ...
                switch (param.from.page) {
                    case "w_change_passwd":
                        {
                            args.ID = param.ID;
                            args.data = {
                                key: v_global.data.key
                            };
                        }
                        break;
                }
