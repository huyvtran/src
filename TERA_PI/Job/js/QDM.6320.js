...
var gw_job_process = {
    ...
    ready: function () {
        ...
        var args = { request: [
                ...
                //{ type: "PAGE", name: "사원", query: "dddw_emp" }
                {
                    ...
                    param: [{ argument: "arg_hcode", value: "QDM024" }]
                },
                {
                    type: "INLINE", name: "재발여부",
                    data: [
                        { title: "초발", value: "0" },
                        { title: "재발", value: "1" }
                    ]
        ...
        function start() {
            ...
            if (v_global.logic.supp) {
                gw_com_api.hide("lyrMenu_Main_상신");
                gw_com_api.hide("lyrMenu_Main_통보");
                gw_com_api.hide("lyrMenu_Main_반려");
            }
		}
    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
				...
				{ name: "저장", value: "저장" },
                { name: "통보", value: "결과통보", icon: "기타", updatable: true },
        ...
        var args = {
            ...
            content: {
                ...
                row: [
                    ...
                    {
                        element: [
                            { header: true, value: "부적합현상", format: { type: "label" } },
                ...
                row: [
                    ...
                    },
                    {
                        element: [
                            ...
                            { name: "last_act_seq", hidden: true },
                            { name: "last_astat", hidden: true },
                            { name: "last_chk_stat", hidden: true }
                        ]
                    }
                ]
            }
        ...
        var args = {
            ...
            element: [
                ...
                { header: "결재결과", name: "check_cd", width: 60, align: "center" },
                { header: "결재자", name: "check_user", width: 60, align: "center" },
                { header: "결재일자", name: "check_dt", width: 60, align: "center", mask: "date-ymd" },
    ...
    procedure: function () {
        ...
        //----------
        var args = { targetid: "lyrMenu_Main", element: "상신", event: "click", handler: click_lyrMenu_Main_상신 };
        gw_com_module.eventBind(args);
        function click_lyrMenu_Main_상신(ui) {
        ...
        //----------
        var args = { targetid: "lyrMenu_Main", element: "반려", event: "click", handler: click_lyrMenu_Main_반려 };
        gw_com_module.eventBind(args);
        function click_lyrMenu_Main_반려(ui) { processSend(ui); }
        //----------
        ...
        v_global.logic.key = "";
        v_global.logic.issue_yn = true;
        if (v_global.process.param != "") {	// Page Parameter 변수 저장
            v_global.logic.key = gw_com_api.getPageParameter("rqst_no");
            v_global.logic.act_seq = gw_com_api.getPageParameter("act_seq");
            if (v_global.logic.act_seq == undefined || v_global.logic.act_seq == "undefined" || v_global.logic.act_seq == "") v_global.logic.act_seq = 0
            ...
            if (v_global.logic.issue_no != "") {
                ...
                // SPC 제외
                if (v_global.logic.root_tp == "QC") {
                    v_global.logic.issue_yn = false;
                }

                if (v_global.logic.issue_yn) {
...
function createTableObject(param) {
    ...
    if (param.objId == "frmData_D1") {
                ...
                row: [
	                {
	                    element: [
	                        { header: true, value: "처리상태", format: { type: "label" } },
	                        {
	                            ...
	                            editable: { type: "select", data: { memory: "처리상태" }, validate: { rule: "required", message: "처리상태" } }
	                        },
                            { name: "astat_dt", hidden: true },
                            { name: "astat_user", hidden: true },
	                        { header: true, value: "계획자", format: { type: "label" } },
	                        {
	                            name: "plan_user_nm", mask: "search", display: false,
	                            editable: { type: "text", validate: { rule: "required", message: "계획자" } }
	                        },
	                        { header: true, value: "계획일자", format: { type: "label" } },
	                        {
	                            name: "plan_dt", mask: "date-ymd",
	                            editable: { type: "text", validate: { rule: "required", message: "계획일자" } }
	                        }
	                    ]
	                },
	                {
	                    element: [
	                        { header: true, value: "처리방안", format: { type: "label" } },
	                        {
	                            ...
	                            editable: { type: "select", data: { memory: "처리방안" }, validate: { rule: "required", message: "처리방안" } }
	                        },
	                        { header: true, value: "처리자", format: { type: "label" } },
	                        { name: "act_user_nm", mask: "search", display: true, editable: { type: "text" }, hidden: v_global.logic.supp },
	                        { name: "act_user", editable: { type: "text", maxlength: (v_global.logic.supp ? 10 : 20) }, hidden: !v_global.logic.supp },
	                ...
	                },
                    {
                        element: [
                            ...
                            {
                                name: "rcr_yn",
                                format: { type: "select", data: { memory: "재발여부" } },
                                editable: { type: "select", data: { memory: "재발여부" } }
                            },
                            { header: true, value: "재발횟수", format: { type: "label" } },
                            { name: "rcr_cnt", editable: { type: "text" }, mask: "numeric-int" },
                            { header: true, value: "원인유형", format: { type: "label" } },
                            {
                                name: "src_tp",
                                format: { type: "select", data: { memory: "원인유형" } },
                                editable: { type: "select", data: { memory: "원인유형" } }
                            }
                        ]
                    },
                    {
                        element: [
                            { header: true, value: "부품 제작일", format: { type: "label" } },
                            {
                                name: "made_date", mask: "date-ymd",
                                editable: { type: "text" }
                            },
                            { header: true, value: "부품 납품일", format: { type: "label" } },
                            {
	                ...
	                {
	                    element: [
	                        { header: true, value: "부적합 원인", format: { type: "label" } },
	                        {
	                            name: "rmk01", style: { colspan: 5 },
	                            format: { type: "textarea", rows: 5, width: 1000 },
	                            editable: { type: "textarea", rows: 5, width: 1000 }
	                        }
	                    ]
	                },
	                {
	                    element: [
	                        { header: true, value: "조치 수리 내용", format: { type: "label" } },
	                        {
	                            name: "rmk02", style: { colspan: 5 },
	                            format: { type: "textarea", rows: 5, width: 1000 },
	                            editable: { type: "textarea", rows: 5, width: 1000 }
	                        }
	                    ]
	                ...
	                {
	                    element: [
	                        { header: true, value: "재발방지대책", format: { type: "label" } },
	                        {
	                            name: "rmk03", style: { colspan: 5 },
	                            format: { type: "textarea", rows: 5, width: 1000 },
	                            editable: { type: "textarea", rows: 5, width: 1000 }
	                        }
	                    ]
	                },
                    {
                        element: [
                            ...
                            {
                                name: "chk_rmk", style: { colspan: 5 },
                                format: { type: "text", width: 1000 },
                                editable: { type: "text", width: 1000 }
                            },
        ...
        args = {
            targetid: "frmData_D2", query: "QDM_6220_D_2", type: "TABLE", title: "유효성 확인",
                ...
                row: [
                    {
                        element: [
                            ...
                            //{ name: "gw_stat_dt", format: { type: "text", width: 200 } }
                            { name: "astat_dt", format: { type: "text", width: 200 } }
                        ]
                    },
                    {
                        element: [
                            { header: true, value: "확인 비고", format: { type: "label" } },
                            {
                                name: "check_rmk", style: { colspan: 7 },
                                format: { type: "textarea", rows: 8, width: 1000 }
                            },
                            { name: "rqst_no", hidden: true },
                            { name: "act_seq", hidden: true },
...
function eventItemChanged(ui) {
    ...
    } else if (ui.element == "astat") {
        gw_com_api.setValue(ui.object, ui.row, "astat_dt", gw_com_api.getDate());
        gw_com_api.setValue(ui.object, ui.row, "astat_user", gw_com_module.v_Session.USR_ID);
        if (gw_com_api.getValue(ui.object, ui.row, "plan_user", ui.type == "GRID" ? true : false) == "") {
            gw_com_api.setValue(ui.object, ui.row, "plan_user_nm", gw_com_module.v_Session.USR_NM, ui.type == "GRID" ? true : false);
            gw_com_api.setValue(ui.object, ui.row, "plan_user", gw_com_module.v_Session.USR_ID, ui.type == "GRID" ? true : false);
        }
    } else if (ui.element == "plan_user_nm") {
        if (gw_com_api.getValue(ui.object, ui.row, "plan_dt", ui.type == "GRID" ? true : false) == "") {
            gw_com_api.setValue(ui.object, ui.row, "plan_dt", gw_com_api.getDate(), ui.type == "GRID" ? true : false);
        }
    } else if (ui.element == "act_user_nm") {
        if (gw_com_api.getValue(ui.object, ui.row, "act_dt", ui.type == "GRID" ? true : false) == "") {
            gw_com_api.setValue(ui.object, ui.row, "act_dt", gw_com_api.getDate(), ui.type == "GRID" ? true : false);
        }
    }
...
function processRetrieve(param) {
    ...
    if (param.object == "grdRoot_SUB_AS" || param.object == "grdRoot_SUB_PO" || param.object == "grdRoot_SUB_QD") {
        if (!v_global.logic.issue_yn) return;
        ...
        args = {
            ...
            key: [{
                KEY: [
                    { NAME: "rqst_no", VALUE: v_global.logic.key },
                    { NAME: "act_seq", VALUE: v_global.logic.act_seq }
                ],
                QUERY: "QDM_6220_S_1"
            }]
        };
...
function processLink(param) {
    ...
    if (param.object == "grdData_Sub") {
	    ...
	    var ChkStat = gw_com_api.getValue("grdData_Sub", "selected", "check_cd", true);
	    var AStat = gw_com_api.getValue("grdData_Sub", "selected", "astat", true);
	    var ReadOnly = false;
	    ...
	    if (!(SelDept == gw_com_module.v_Session.DEPT_CD || SelUser == gw_com_module.v_Session.USR_ID ||
            SelSupp == gw_com_module.v_Session.USR_ID || gw_com_module.v_Session.USER_TP == "SYS")) {
	    ...
	    //if (AStat == "담당이관") {
	    //    // 처리상태를 "담당이관" 선택 후 새로운 담당자 지정 완료 시 수정 불가
	    //    var NewSeq = Number(SelSeq) + 1;
	    //    if (gw_com_api.getFindRow("grdData_Sub", "act_seq", NewSeq) > 0) {
	    //        gw_com_module.v_Option.authority.control == "R";
	    //        ReadOnly = true;
	    //    }
        //}
        ...
        // ACT_SEQ != MAX(ACT_SEQ)
	    if (SelSeq != gw_com_api.getValue("frmData_Main", 1, "last_act_seq")) {
	        gw_com_module.v_Option.authority.control == "R";
	        ReadOnly = true;
        }

	    createTableObject({ objType: "FORM", objId: "frmData_D1", readonly: ReadOnly, clear: true });
...
function processInsert(ui) {
    ...
    if (ui.object == "lyrMenu_Sub") {
        ...
        var args = {
            ...
            width: 600, height: 450, scroll: true, open: true, control: true, locate: ["center", "top"]
        };
...
function processSave(param) {
    ...
    //// 처리상태가 처리완료일 경우 첨부 필수
    //if (gw_com_api.getValue("frmData_D1", 1, "astat") == "처리완료") {
    //    if (gw_com_api.getRowCount("grdData_File1") < 1) {
    //        gw_com_api.messageBox([{ text: "첨부문서를 등록하시기 바랍니다." }]);
    //        //return false;
    //    }
    //}
...
function successSave(response, param) {
        ...
        $.each(this.KEY, function () { 
        	switch (this.NAME) {
        	    case "rqst_no":
        	        ...
        	        break;
        	    case "act_seq":
        	        v_global.logic.act_seq = this.VALUE;
        	        break;
        	}
        });
    });
    ...
    // 반려 메일 발송
    if (gw_com_api.getValue("frmData_Main", 1, "last_astat") == "반려") {
        var param = {
            type: "NCR-REJECT3",
            key_no: v_global.logic.key,
            key_seq: gw_com_api.getValue("frmData_Main", 1, "last_act_seq")
        };
        var args = {
            url: "COM",
...
function processSend(param) {
    ...
    if (param.element == "통보") {
        //gw_com_api.getValue("grdData_Sub", "selected", "check_cd", true);
        if (gw_com_api.getValue("frmData_Main", 1, "last_chk_stat") != "적합") {
            gw_com_api.messageBox([
                { text: "확인결과가 적합이 아닌 경우에는 결과통보를 할 수 없습니다." }], 480);
            return;
        }
    }

    var type = param.element == "통보" ? "NCR-FINAL" : "NCR-REJECT2";
    gw_com_api.messageBox([
        { text: "시정조치 처리에 대한 이메일을 발송합니다." + "<br>" },
        { text: "계속 하시겠습니까?" }
    ], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", { type: type });
...
function processApprove(param) {
    ...
    var stat = gw_com_api.getValue("frmData_Main", 1, "last_astat");    // gw_com_api.getValue("grdData_Sub", "selected", "astat", true);
    if (stat != "처리완료") {
        gw_com_api.showMessage("처리가 완료되지않아 상신할 수 없습니다.");
        return;
    }

    stat = gw_com_api.getValue("frmData_Main", 1, "last_chk_stat");     // gw_com_api.getValue("grdData_Sub", "selected", "check_cd", true);    // check_cd = QDM_ISSUE_D3.astat
    if (stat == "상신" || stat == "적합") {
        gw_com_api.showMessage("결재가 진행중이거나 완료되었습니다.");
        return;
    }

    var seq = gw_com_api.getValue("frmData_Main", 1, "last_act_seq");   // gw_com_api.getValue("grdData_Sub", "selected", "act_seq", true);
    var title = encodeURIComponent(Query.getApprovalTitle({ type: "NCR", ref_key1: v_global.logic.key, ref_key2: seq }));
    var url = "http://gw.terasemicon.com/_ERPR/Public/ERPRLogin.aspx?UserId=" + gw_com_module.v_Session.USR_ID
        + "&Pwd=&Lang=ko&EAID=112&gw_num=100&htmltag=&ncr_no=" + v_global.logic.key + "&ncr_seq=" + seq + "&NextUrl=/_EAPP/EADocumentWrite.aspx?FormID=112&ErpDocTitle=" + title;
