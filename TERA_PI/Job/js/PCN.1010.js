...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
                { header: "협력사명", name: "comp_nm", width: 120, hidden: v_global.logic.supp_yn },
                { header: "작성일시", name: "issue_dt", width: 100, align: "center" },
...
function processItemchanged(param) {
    ...
    if (param.object=="frmData_MAIN") {
        ...
        } else if (param.element == "comp_nm" && param.value.current == "") {
            gw_com_api.setValue(param.object, param.row, "comp_cd", "");
        }
...
function processItemdblclick(param) {
    ...
    } else if (param.element == "item_cd" || param.element == "item_cd2" || param.element == "item_nm") {
        v_global.event.type = param.type;
        v_global.event.object = param.object;
        v_global.event.row = param.row;
        v_global.event.element = param.element;
        if (param.element == "item_cd" || param.element == "item_cd2")
            v_global.event.data = {
                part_cd: gw_com_api.getValue(param.object, param.row, param.element, (param.type == "GRID" ? true : false))
            };
        else
            v_global.event.data = {
                part_nm: gw_com_api.getValue(param.object, param.row, param.element, (param.type == "GRID" ? true : false))
            };
        if (v_global.logic.supp_yn) {
            v_global.event.data.supp_cd = v_global.logic.supp_cd;
            var args = {
            ...
            var args = {
                ...
                width: 800, height: 460, locate: ["center", 100], open: true,
    ...
    } else if (param.element == "comp_nm") {
        if (v_global.logic.supp_yn) return;
        v_global.event.type = param.type;
        v_global.event.object = param.object;
        v_global.event.row = param.row;
        v_global.event.element = param.element;
        v_global.logic.search = {
            supp_nm: gw_com_api.getValue(param.object, param.row, param.element)
        };
    ...
    if (param.object == "lyrMenu_1") {
        ...
        var args = {
            ...
            data: [
                ...
                { name: "astat", value: "작성" },
                { name: "pstat", value: "작성" },
                { name: "rqst_user_nm", value: v_global.logic.supp_yn ? "" : gw_com_module.v_Session.USR_NM },
                { name: "rqst_user_pos", value: v_global.logic.supp_yn ? "" : gw_com_module.v_Session.POS_NM },
                { name: "rqst_user_email", value: v_global.logic.supp_yn ? "" : gw_com_module.v_Session.USR_ID + "@terasemicon.com" }
            ],
            clear: [
                { type: "FORM", id: "frmData_MEMO1" },
                { type: "FORM", id: "frmData_MEMO2" },
...
function createDW(param) {
            ...
            row: [
                ...
                },
                {
                    element: [
                        ...
                        { name: "item_cd2", editable: { type: "hidden", width: 166 } },
                ...
                },
                {
                    element: [
                        { header: true, value: "제목", format: { type: "label" } },
                        {
                            ...
                            editable: { type: "hidden", width: 1012 }
                        }
                    ]
                },
                {
    ...
    if (edit_yn) {
        if (!v_global.logic.supp_yn) {
            args.content.row[0].element[3].editable = { type: "text", width: 416, maxlength: 100, validate: { rule: "required" } };             //협력사
            args.content.row[0].element[3].mask = "search";                                                                                     //협력사
        }
        ...
        args.content.row[4].element[1].editable = { type: "text", width: 1012, maxlength: 100, validate: { rule: "required" } };                //제목
        args.content.row[5].element[1].editable = { type: "textarea", rows: 4, width: 1010 };                                                   //변경사유
    }
    //----------
    gw_com_module.formCreate(args);
    //----------
    // 수기입력 가능하도록 변경
    if (!v_global.logic.supp_yn) {
        $("#frmData_MAIN_comp_nm").attr("readonly", false);
        $("#frmData_MAIN_item_cd").attr("readonly", false);
    }
    //=====================================================================================
    var args = {
    ...
    //----------
    if (edit_yn) {
        args.content.row[0].element[1].editable = { type: "textarea", rows: 4, width: 1010 };   //품질검증내용
    }
    //----------
    gw_com_module.formCreate(args);
    //=====================================================================================
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
                ...
                switch (param.from.page) {
                    case "PCN_1011":
                        if (param.data != undefined) {
                            ...
                            gw_com_api.setValue("frmData_MAIN", 1, "item_cd2", param.data.item_cd, false, false, true);
                        }
                        break;
                }
                closeDialogue({ page: param.from.page });
        ...
        case gw_com_api.v_Stream.msg_selectedSupplier:
            {
                var supp_cd = (v_global.event.object == "frmOption" ? "supp_cd" : "comp_cd");
                var supp_nm = (v_global.event.object == "frmOption" ? "supp_nm" : "comp_nm");
                gw_com_api.setValue(v_global.event.object,
			                        v_global.event.row,
			                        supp_cd,
			                        param.data.supp_cd,
