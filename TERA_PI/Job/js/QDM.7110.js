...
var gw_job_process = {
    ...
    UI: function () {
        ...
        //=====================================================================================
        var args = {
            targetid: "lyrMenu_IMAGE_A", type: "FREE",
            element: [
        ...
        var args = {
            ...
            caption: true, height: 150, pager: false, show: true, selectable: true, number: true, //multi: true, checkrow: true,
            color: { row: true },
            element: [
                ...
                row: [
                    {
                        element: [
                            ...
                            { name: "proj_nm", editable: { type: "text", validate: { rule: "required" } }, mask: "search" },
                            { header: true, value: "점검일자", format: { type: "label" } },
                            { name: "qc_date", editable: { type: "text", width: 90, validate: { rule: "dateISO" } }, mask: "date-ymd" },
                            { header: true, value: "담당자", format: { type: "label" } },
                            { name: "qc_emp_nm", editable: { type: "text", width: 60, validate: { rule: "required" } }, mask: "search", style: { colfloat: "float" } },
                    ...
                    },
                    {
                        element: [
                            { header: true, value: "검사단계", format: { type: "label" } },
                            {
                                ...
                                editable: { type: "select", data: { memory: "검사단계", unshift: [{ title: "-", value: "" }] }, validate: { rule: "required" } }
                            },
                    ...
                    //            editable: { type: "textarea", rows: 3, maxlength: 500, width: 982, validate: { rule: "required" } }
                    //        }
                    //    ]
                    //}
                ]
    ...
    procedure: function () {
        ...
        gw_com_module.eventBind(args);
        //=====================================================================================
        var args = { targetid: "lyrMenu_IMAGE_A", element: "추가", event: "click", handler: processButton };
        gw_com_module.eventBind(args);
        //----------
        var args = { targetid: "lyrMenu_IMAGE_A", element: "삭제", event: "click", handler: processButton };
        gw_com_module.eventBind(args);
        //=====================================================================================
...
function processInsert(param) {
    ...
    if (param.object == "lyrMenu") {
        ...
        if (param.element == "추가") {
            ...
            if (row > 0) {
                data.push({ name: "projkey", value: gw_com_api.getValue("grdList_MAIN", row, "projkey", true) });
                data.push({ name: "proj_nm", value: gw_com_api.getValue("grdList_MAIN", row, "proj_nm", true) });
                //data.push({ name: "qc_date", value: gw_com_api.getValue("grdList_MAIN", row, "proj_nm", true) });
                data.push({ name: "qc_dept", value: gw_com_api.getValue("grdList_MAIN", row, "qc_dept", true) });
                data.push({ name: "qc_dept_nm", value: gw_com_api.getValue("grdList_MAIN", row, "qc_dept_nm", true) });
                data.push({ name: "qc_emp", value: gw_com_api.getValue("grdList_MAIN", row, "qc_emp", true) });
                data.push({ name: "qc_emp_nm", value: gw_com_api.getValue("grdList_MAIN", row, "qc_emp_nm", true) });
                data.push({ name: "qc_tp", value: gw_com_api.getValue("grdList_MAIN", row, "qc_tp", true) });
                //data.push({ name: "qc_tp_nm", value: gw_com_api.getValue("grdList_MAIN", row, "qc_tp_nm", true) });
                data.push({ name: "qc_fg", value: gw_com_api.getValue("grdList_MAIN", row, "qc_fg", true) });
                data.push({ name: "qc_cnt", value: gw_com_api.getValue("grdList_MAIN", row, "qc_cnt", true) });
                data.push({ name: "ins_dt", value: "*신규" });
            }
            var args = {
            ...
            var args = {
                type: "PAGE", page: "QDM_7112", title: "출하검사내역 일괄등록",
                width: 1100, height: 525, open: true, locate: ["center", "top"]
            }; //

            if (gw_com_module.dialoguePrepare(args) == false) {
                var args = {
                    page: "QDM_7112",
                    param: { ID: gw_com_api.v_Stream.msg_openDialogue }
                };
...
function processDelete(param) {
    ...
    if (param.object == "lyrMenu") {
        ...
        var file_id = gw_com_api.getValue("frmData_IMAGE_A", 1, "file_id1");
        if (key == "" || file_id == "") {
...
function processSave(param) {
    ...
    if (gw_com_api.getValue("frmData_MAIN", 1, "qc_memo1") == "") {
        gw_com_api.messageBox([{ text: "부적합현상 내용을 입력하세요." }]);
        return false;
    }
...
function processSendmail(param) {
    ...
    var args = {
        type: "PAGE", page: "QDM_7111", title: "출하검사내역 통보",
    ...
    v_global.logic.search = {
        ...
        qc_emp_nm: gw_com_api.getValue("frmOption", 1, "qc_emp_nm"),
        qc_emp: gw_com_api.getValue("frmOption", 1, "qc_emp"),
        qc_dept_nm: gw_com_api.getValue("frmOption", 1, "qc_dept_nm"),
        qc_dept: gw_com_api.getValue("frmOption", 1, "qc_dept"),
        proj_nm: gw_com_api.getValue("frmOption", 1, "proj_nm"),
                                ...
                                param: [
                                    {
                                        ...
                                        row: [{
                                            ...
                                            column: [
                                                { name: "pchk_no", value: key },
                                                { name: "qc_image1", value: param.data[0].KEY[0].VALUE }
                                            ]
                                        }]
                                    }
                                ]
