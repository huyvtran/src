...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
				{ name: "조회", value: "조회" },
                { name: "발생", value: "접수/반려취소", icon: "아니오" },
				{ name: "닫기", value: "닫기" }
            ]
        };
        //----------
        gw_com_module.buttonMenu(args);
        gw_com_api.hide("lyrMenu2");
        //=====================================================================================
        var args = {
        ...
        //=====================================================================================
        var args = {
            targetid: "grdData_현황", query: "QDM_6110_M_1", title: "발생 내역",
            height: 400, show: true, selectable: true, number: true,
            //editable: { master: true, bind: "select", focus: "astat", validate: true },
            element: [
				{
				...
				{ hidden: true, header: "Project No.", name: "proj_no", width: 70, align: "center" },
				...
				{
				    hidden: true, header: "처리유형", name: "act_tp", width: 60, align: "center",
				    editable: { type: "select", data: { memory: "처리유형", unshift: [{ title: "전체", value: ""}] } }
                },
				{ header: "접수비고", name: "astat_rmk", width: 200, align: "left", editable: { type: "text" }, hidden: true },
    ...
    procedure: function () {
        ...
        gw_com_module.eventBind(args);
        //=====================================================================================
        var args = { targetid: "lyrMenu2", element: "조회", event: "click", handler: click_lyrMenu_조회 };
        gw_com_module.eventBind(args);
        //----------
        var args = { targetid: "lyrMenu2", element: "발생", event: "click", handler: processButton };
        gw_com_module.eventBind(args);
        //----------
        var args = { targetid: "lyrMenu2", element: "닫기", event: "click", handler: click_lyrMenu_닫기 };
        gw_com_module.eventBind(args);
        //=====================================================================================
        ...
        function rowselected_grdData_현황(ui) {
            ...
            if (gw_com_api.getValue(ui.object, ui.row, "pstat", true) == "발생") {
...
function processButton(param) {
    ...
    switch (param.element) {
        ...
        case "발생":
            {
                if (gw_com_api.getSelectedRow("grdData_현황") < 1) return;
                var issue_no = gw_com_api.getValue("grdData_현황", "selected", "issue_no", true);
                var stat = Query.getStat({ issue_no: issue_no });
                var astat = param.element;
                if (stat.astat == astat) return;
                if (stat.d1_pstat == "" || stat.d1_pstat == undefined || $.inArray(param.element, ["접수", "발생"]) >= 0) {
                    var args = {
                        url: "COM",
                        user: gw_com_module.v_Session.USR_ID,
                        param: [{
                            query: gw_com_api.getAttribute("grdData_현황_data", "query"),
                            row: [{
                                crud: "U",
                                column: [
                    ...
                    gw_com_api.messageBox([{ text: "이미 발행되거나 발행중인 내역입니다." }]);
                }
            }
            break;
    }
...
function successSave(response, param) {
    ...
    if (param != undefined) {
        if (param.astat == "반려") {
            ...
            var args = {
                url: "COM",
                subject: Query.getSubject({ type: "ISSUE-REJECT", issue_no: param.issue_no }),
                body: Query.getBody({ type: "ISSUE-REJECT", issue_no: param.issue_no }),
                to: Query.getTo({ type: "ISSUE-REJECT", issue_no: param.issue_no }),
                edit: true
            };
...
function popupDetail(ui) {
    ...
    var args = {
        ...
        width: 1100, height: 600, resize: true, scroll: true, open: true, control: true, locate: ["center", "top"]
    };
    ...
    getStat: function (param) {
        ...
        var args = {
            ...
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    "?QRY_ID=QDM_6110_M_1" +
                    "&QRY_COLS=astat,d1_pstat" +
                    "&CRUD=R" +
                    "&arg_issue_tp=&arg_cust_cd=&arg_cust_dept=&arg_cust_prod_nm=&arg_astat=&arg_ymd_fr=0&arg_ymd_to=99999999&arg_dept_area=&arg_issue_no=" + param.issue_no,
    ...
    getSubject: function (param) {
        ...
        var args = {
            ...
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    ...
                    "&QRY_COLS=val" +
                    "&CRUD=R" +
                    "&arg_type=" + param.type + "&arg_field=subject&arg_key_no=" + param.issue_no + "&arg_key_seq=0",
            handler_success: successRequest
        };
        //=================== async : false ========================
        $.ajaxSetup({ async: false });
    ...
    getBody: function (param) {
        ...
        var args = {
            ...
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    ...
                    "&QRY_COLS=val" +
                    "&CRUD=R" +
                    "&arg_type=" + param.type + "&arg_field=body&arg_key_no=" + param.issue_no + "&arg_key_seq=0",
            handler_success: successRequest
        };
        //=================== async : false ========================
        $.ajaxSetup({ async: false });
    ...
    getTo: function (param) {
        ...
        var args = {
            ...
            url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                    ...
                    "&QRY_COLS=name,value" +
                    "&CRUD=R" +
                    "&arg_type=" + param.type + "&arg_key_no=" + param.issue_no + "&arg_key_seq=0",
            handler_success: successRequest
        };
        //=================== async : false ========================
        $.ajaxSetup({ async: false });
