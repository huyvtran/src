...
var gw_job_process = {
    ...
    ready: function () {
        ...
        var args = {
            request: [
                ...
                {
                    type: "INLINE", name: "협력사명",
                    data: [{ title: gw_com_module.v_Session.USR_NM, value: gw_com_module.v_Session.EMP_NO }]
                },
                {
                    type: "INLINE", name: "보기",
                    data: [
    ...
    UI: function () {
                        ...
                        element: [
                            {
                                ...
                                editable: { type: "select", size: 7, maxlength: 20, data: { memory: "품목분류", unshift: [{ title: "전체", value: "" }] } }
                            },
                            {
                                name: "qcitem_cd", label: { title: "검사항목 :" },
                                editable: { type: "select", size: 7, maxlength: 20, data: { memory: "검사항목", unshift: [{ title: "전체", value: "" }] } }
                            }
                        ]
                    },
                    {
        ...
        //==== Grid : Main ====
        var args = {
            targetid: "grdData_내역", query: "SPC_2000_M_1", title: "검사 내역",
            caption: true, width: 570, height: 494, show: true, selectable: true,
            element: [
				...
				{ header: "횡전개", name: "repair_yn", width: 40, align: "center" },
        ...
        var args = {
            ...
            content: {
                width: { label: 100, field: 180 }, height: 25,
                row: [
                    {
                        element: [
                              { header: true, value: "협 력 사", format: { type: "label" } },
                              { name: "supp_nm", editable: { type: "text", validate: { rule: "required" } }, mask: "search" },
                              { name: "supp_cd", editable: { type: "hidden" }, hidden: true },
                              { header: true, value: "등록번호", format: { type: "label" } },
                              { name: "qc_seq", editable: { type: "hidden" } }
                        ]
                    },
                    {
                        element: [
                    ...
                    },
                    {
                        element: [
                              ...
                              { name: "qc_charge", editable: { type: "text", maxlength: 10, validate: { rule: "required" } } },
        ...
        gw_com_module.buttonMenu(args);
        //=====================================================================================
        var args = { targetid: "grdData_결과", query: "SPC_2000_S_2", title: "검사 결과",
            ...
            element: [
                {
                    ...
                    editable: { type: "text", validate: { rule: "required", message: "Serial No." } }
                },
                {
                    header: "Value.", name: "qc_value", width: 80,
                    editable: { type: "text" }
                },
                {
                ...
                { header: "측정자 비고", name: "qc_rmk", width: 200, align: "left" , editable: { type: "text"} },
                { hidden: true, header: "품질 비고", name: "rmk1", width: 120, align: "left" },
                { hidden: true, header: "부품 비고", name: "rmk2", width: 120, align: "left" },
...
function checkRemovable(param) {
    ...
    if (status == "initialize" || status == "create")
        ...
        if (gw_com_api.getSelectedRow("grdData_결과") == null) {
            gw_com_api.messageBox([
...
function processItemchanged(param) {
    ...
    } else if (param.object == "frmData_상세") {
        switch (param.element) {
            case "supp_nm":
                gw_com_api.setValue("frmOption", 1, param.element, param.value.current);
                gw_com_api.setValue("frmOption", 1, "supp_cd", gw_com_api.getValue(param.object, param.row, "supp_cd"));
                break;
        }
    }
...
function processInsert(param) {
    ...
    if (param.sub) {
	    ...
	    gw_com_api.selectRow("grdData_내역", "reset");
	    var args = {
	        targetid: "frmData_상세", edit: true, updatable: true,
	        data: [
	            { name: "supp_cd", value: gw_com_api.getValue("frmOption", 1, "supp_cd") },
	            { name: "supp_nm", value: gw_com_api.getValue("frmOption", 1, "supp_nm") },
	            { name: "qc_date", value: gw_com_api.getDate() },
                { name: "repair_yn", value: "0" },
                { name: "qc_charge", value: (v_global.logic.Supp ? "" : gw_com_module.v_Session.USR_NM) }
	        ],
	        clear: [
			    { type: "GRID", id: "grdData_결과" }
	        ]
	    ...
	    if (gw_com_api.getValue("frmData_상세", 1, "supp_cd") == "") {
	        var args = {
	            object: "frmData_상세",
	            element: "supp_nm",
	            row: 1,
...
function processRemove(param) {
    ...
    if (param.sub) {
	    args = {
	        url: "COM",
	        target: [
			    { type: "GRID", id: "grdData_결과", key: [{ row: "selected", element: [{ name: "qc_seq" }, { name: "sub_seq" }] }] }
	        ],
