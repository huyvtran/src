    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
				...
				{ name: "삭제", value: "납품서 삭제" },
				{ name: "저장", value: "납품서 저장" },
        ...
        //=====================================================================================
        var args = {
            targetid: "grdList_Main", query: "SRM_4110_M_1", title: "납품 현황",
            height: "100%", show: true, selectable: true, number: true, pager: false, // dynamic: true, multi: true, checkrow: true,
            element: [
        ...
        $("#grdList_Main_data").parents('div.ui-jqgrid-bdiv').css("max-height", "100px");
        //=====================================================================================
        var args = {
...
function processItemChanged(param) {
    ...
    var vl = param.value.current;

    if (param.element == "Remark") {   // 복수행 입력란의 개행문자 치환
        vl = vl.replace(/\r\n/g, "CRLF");
        gw_com_api.setValue("grdData_Sub", "selected", param.element, vl, true);
    }
    ...
    if (param.object == "frmOption2") {
        ...
        if (param.element == "dlv_qty") {
            var pur_qty = gw_com_api.getValue(param.object, param.row, "pur_qty", true);
            if (Number(pur_qty) < Number(param.value.current)) {
                gw_com_api.messageBox([{ text: "납품수량이 잘못 입력되었습니다." }]);
                gw_com_api.setValue(param.object, param.row, param.element, (pur_qty < param.value.prev ? pur_qty : param.value.prev), true);
            } else if (param.value.current == 0) {
                gw_com_api.messageBox([{ text: "납품수량이 잘못 입력되었습니다." }]);
            }
        } else if (param.element == "direct_yn") {
            ...
            //} else {
            //    gw_com_api.setValue(param.object, param.row, "dir_date", "", param.type == "GRID" ? true : false);
            //    gw_com_api.setValue(param.object, param.row, "dir_place", "", param.type == "GRID" ? true : false);
            //}
        }
    }
...
function popupFindItem(param) {
    ...
    switch (param.element) {
        case "추가": {
            ...
            v_global.logic.poup_param = {
                supp_cd: gw_com_api.getValue("frmOption", 1, "supp_cd"),
                pur_no: gw_com_api.getRowCount("grdData_Sub") > 0 ? gw_com_api.getValue("grdData_Sub", 1, "pur_no", true) : gw_com_api.getValue("frmOption", 1, "pur_no"),
                except: chkRows
            }
            if (gw_com_module.dialoguePrepare(args) == false) {
                var args = {
                    ...
                    param: {
                        ID: gw_com_api.v_Stream.msg_selectPart_SCM,
                        data: v_global.logic.poup_param
                    }
                };
                gw_com_module.dialogueOpen(args);
            }
...
function processDelete(param) {
    ...
    else if (param.object == "lyrMenu_Main") {
        if (!checkManipulate({})) return;
        ...
        case gw_com_api.v_Stream.msg_openedDialogue: {
            ...
            switch (param.from.page) {
                ...
                case "w_find_srm_dlvitem2": {
                    ...
                    args.data = v_global.logic.poup_param;
                } break;
                case "SRM_4111": {
                    args.ID = gw_com_api.v_Stream.msg_selectPart_SCM;
                    args.data = {
