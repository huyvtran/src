...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = {
            ...
            height: 450, show: true, selectable: true, number: true, // dynamic: true, multi: true, checkrow: true,
            editable: { master: true, bind: "select", focus: "dept_area", validate: true },
            element: [
				{
				    header: "협력사코드", name: "supp_cd", width: 50,
				    editable: { type: "hidden", validate: { rule: "requied" }, width: 118 }
				},
                ...
                { header: "주소", name: "addr", width: 160, align: "left" },
                {
                    ...
                    editable: { type: "select", data: { memory: "검수유형" }, width: 68, validate: { rule: "required" } }
                },
                {
                    header: "성적서", name: "chk01_yn", width: 40, align: "center",
                    format: { type: "select", data: { memory: "성적서" } },
                    editable: { type: "select", data: { memory: "성적서" }, width: 68, validate: { rule: "required" } }
                },
                {
                    header: "사업부", name: "dept_area", width: 50, align: "left",
                    format: { type: "select", data: { memory: "사업부" } },
                    editable: { type: "hidden" }, hidden: true
                },
                { header: "최종수정자", name: "upd_usr_nm", width: 40, align: "center" },
                { header: "최종수정일시", name: "upd_dt", width: 100, align: "center" }
            ]
        };
...
function processFind(param) {
    ...
    switch (param.element) {
        case "supp_cd":
            {
                var args = {
                    type: "PAGE", page: "w_find_supplier_qa", title: "협력사 검색",
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        ...
        case gw_com_api.v_Stream.msg_selectedSupplier_User:
            {
                var find = gw_com_api.getFindRow("grdData_Main", "supp_cd", param.data.user_id);
                if (find > 0) return;
                var args = {
                    ...
                    data: [
                        ...
                        { name: "addr", value: param.data.addr1 + " " + param.data.addr2 },
                        { name: "check_yn", value: "1" },
