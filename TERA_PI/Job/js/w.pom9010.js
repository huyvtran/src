                ...
                row: [
                    {
                        element: [
				            {
				                name: "area_cd", label: { title: "사업부 :" }, hidden: true,
				                editable: {
				                    type: "select",
				                    data: { memory: "사업부", unshift: [{ title: "전체", value: "%" }] }
				                }
				            },
				            {
        ...
        var args = {
            ...
            element: [
				...
				},
				{
				    ...
				    editable: { type: "text", validate: { rule: "required", message: "사용자명" }, width: 330 }
				},
				{
				...
				{ header: "E-Mail", name: "email", width: 170 },
				{ header: "휴대폰번호", name: "hp_no", width: 100, align: "center" },
                { header: "최종접속일", name: "on_date2", width: 110, align: "center" },
    ...
    if (param.object == "grdData_협력사") {
        if (param.element == "login_id") {
            if (gw_com_api.getCRUD(param.object, param.row, true) == "create") {
                gw_com_api.setValue(param.object, param.row, "user_pw", hex_md5(param.value.current), true, true, true);
            }
        }
    }
...
function processResetPW(param) {
    ...
    gw_com_api.setValue("grdData_협력사", "selected", "user_pw", hex_md5(login_id), true, true, true);
    ...
    var rows = [];
    $.each(ids, function () {
        var login_id = gw_com_api.getValue("grdData_협력사", this, "login_id", true);
        rows.push({
            crud: "U",
            column: [
                { name: "user_id", value: gw_com_api.getValue("grdData_협력사", this, "user_id", true) },
                { name: "user_pw", value: hex_md5(login_id) }
            ]
        })
    });
    var qryZfile = {
        nomessage: true,
        query: "w_pom9010_M_1", // ZFILE
        row: rows
    };
        ...
        case gw_com_api.v_Stream.msg_selectedSupplier_User:
            {
			    ...
			    gw_com_api.setValue(v_global.event.object, v_global.event.row, "user_pw", hex_md5(param.data.rgst_no), (v_global.event.type == "GRID") ? true : false);
