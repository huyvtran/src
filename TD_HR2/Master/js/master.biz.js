...
var gw_job_process = {
    ...
    ready: function () {
        ...
        function successSession(data) {
            ...
            var content =
                "<img id=\"imgNotice2\" src=\"/Style/images/common/icon/notice.png\" align=\"middle\" onmouseover=\"\" style=\"cursor: pointer;\" />" + "&nbsp;" +
                "<span id=\"txtNotice\" onmouseover=\"\" style='font-family: Verdana; font-weight: bold; border-bottom: 1px solid #777777; color: #777777; vertical-align: middle; cursor: pointer;'>" +
                "공지사항" +
                "</span>" +
                "&nbsp;&nbsp;" +
                "<img id=\"imgUser\" src=\"/Style/images/common/icon/logon_1_16.png\" align=\"middle\" onmouseover=\"\" style=\"cursor: pointer;\" />" + "&nbsp;" +
                "<span id=\"txtUser\" style='font-family: Verdana; font-weight: bold; border-bottom: 1px solid #777777; color: #777777; vertical-align: middle; cursor: pointer;'>" +
                //"◈ " +
            ...
            var qry_id = "PLM_MENU";
            // for APS
            if (v_Session.USER_TP == "EMP" && location.hostname.split(".")[0].toUpperCase() == "PI") {
                qry_id = "PLM_MENU_4_PI";
            }
            ...
            var args = {
                request: "PAGE",
                url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                        "?QRY_ID=" + (v_Session.USER_TP == "SUPP" ? "w_plm_notice_supp" : "w_plm_notice") +
            ...
            var args = {
                ...
                url: "../Service/svc_Data_Retrieve_JSONs.aspx" +
                        "?QRY_ID=DLG_APPR_99" +
                        "&QRY_COLS=appr_id,appr_box,doc_title,rqst_emp_nm,rqst_date2" +
                        "&CRUD=R" +
			            "&arg_user_id=" + v_Session.USR_ID,
                handler_success: successAppr
            };
            gw_com_module.callRequest(args);
        ...
        function successMenu(data) {
            ...
            $.each(data, function (i) {
                if (this.DATA[column.level_no] == 1) {
                    contents =
                        "<li class='has-sub '><a href='#'><span>" + this.DATA[column.menu_nm] + "</span></a>" +
        ...
        function successNotice(data) {
            ...
            $.each(data, function (i) {
                content = content +
                    "<tr style='cursor:pointer;' onclick=\"gw_job_process.notice('" + this.DATA[column.nt_seq] + "');\">" +
        ...
        function successAppr(data) {
            ...
            $.each(data, function (i) {
                content = content +
                    "<tr id='r_appr_" + i + "' style='cursor:pointer;' onclick='gw_job_process.appr_box(\"" + this.DATA[column.appr_box] + "\", \"" + this.DATA[column.appr_id] + "\");'>" +
                    "<td width='380px' align='left'>" +
                    this.DATA[column.doc_title] +
                    "</td>" +
                    "<td width='60px' align='center'>" +
                    this.DATA[column.rqst_emp] +
                    "</td>" +
                    "<td width='70px' align='right'>" +
                    this.DATA[column.rqst_date] +
                    "</td>" +
                    "</tr>";
            });
            $("#tblAppr").html(content);

            //$.each(data, function (i) {
            //    var args = {
            //        targetid: "r_appr_" + i,
            //        event: "click",
            //        handler: click_imgAppr,
            //        element: [
            //            { name: "appr_box", value: this.DATA[column.appr_box] },
            //            { name: "appr_id", value: this.DATA[column.appr_id] }
            //        ]
            //    };
            //    gw_com_module.eventBind(args);
            //});
        ...
        function successPO(data) {
            ...
            $.each(data, function (i) {
                content = content +
                    ...
                    //"</td>" +
                    "<td width='400px'>" +
                    "발주번호: " + this.DATA[column.pur_no] + ", 발주일: " + this.DATA[column.pur_date] + ", 고객요청일: " + this.DATA[column.req_date] +
                    "</td>" +
                    "</tr>";
            });
        ...
        //----------
        function click_imgAppr() {

            gw_job_process.processTab(
            "DLG_APPR_BOX",
            "결재함",
            "../job/DLG_APPR_BOX.aspx",
            "");
        ...
        //----------
        function click_btnPassword() {

            var args = {
                type: "PAGE", page: "DLG_PASSWORD", title: "본인인증 비밀번호 변경",
    ...
    notice: function (seq) {
        ...
        var args = (seq == undefined ? "" : "&nt_seq=" + seq);
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        case gw_com_api.v_Stream.msg_setKey:
            {
                switch (param.data.job) {
                    case "HCEM":
                        {
                            v_Job.HCEM = param.data.param;
                        }
                        break;
                }
            }
                    ...
                    case "DLG_PASSWORD":
                        {
                            ...
                            args.data = { usr_id: v_Session.USR_ID };
                        }
                        break;
                }
                gw_com_module.streamInterface(args);
