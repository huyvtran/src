...
function processSave(param) {
    ...
    if (param.element == "합격" || param.element == "불합격") {
        ...
        var ann_seq = "";
        var app_key = "";
        var rec_stat = param.element == "합격" ? "81" : "91";
        $.each(ids, function () {
            var rowdata = $("#grdData_Main_data").getRowData(this);
            ann_key = rowdata.ann_key;
            ann_seq = rowdata.ann_seq;
            app_key += (app_key == "" ? "" : ",") + rowdata.app_key;
        });

        var email = {
            mail_type: param.element == "합격" ? "HRM_REC01" : "HRM_REC02",
            ann_key: ann_key,
            ann_seq: ann_seq,
            app_key: app_key
        };
        var proc = {
            url: "COM",
            procedure: "sp_updateRECStat",
            nomessage: true,
            input: [
                { name: "ann_key", value: ann_key, type: "varchar" },
                { name: "ann_seq", value: ann_seq, type: "int" },
                { name: "app_key", value: app_key, type: "varchar" },
                { name: "rec_stat", value: rec_stat, type: "varchar" },
...
function successBatch(response, param) {
    ...
    if (gw_com_api.showMessage("통보 메일을 보내시겠습니까?", "yesno")) {
...
function processSendMail(param) {
    ...
    v_global.logic.mail = param;
    var args = {
        url: "COM",
        //to: param.email,
        temp_id: param.mail_type,
        edit: true,
        section: "smtp_2",
    };
    //gw_com_module.sendMail(args);
    window.open("", "pop_email", "scrollbars=no,resizable=no,menubar=no,toolbar=no,width=1000,height=570");
    ...
    $("#" + frmId).attr("action", "../Job/DLG_EMAIL_RECRUIT.aspx?USR_ID=" + gw_com_module.v_Session.USR_ID);
...
//----------
function processSendMail2(param) {

    var proc = {
        url: "COM",
        procedure: "sp_sendMail_REC",
        nomessage: true,
        input: [
            { name: "ann_key", value: v_global.logic.mail.ann_key, type: "varchar" },
            { name: "ann_seq", value: v_global.logic.mail.ann_seq, type: "int" },
            { name: "app_key", value: v_global.logic.mail.app_key, type: "varchar" },
            { name: "subject", value: param.subject, type: "varchar" },
            { name: "body", value: param.body, type: "varchar" }
        ],
        handler: {
            success: successSend,
            param: param
...
//----------
function successSend(response, param) {
