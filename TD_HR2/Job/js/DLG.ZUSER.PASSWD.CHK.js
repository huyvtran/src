...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = {
            ...
            editable: { focus: "passwd", validate: true },
            content: {
                row: [
                    {
                        element: [
                            {
                                name: "passwd", label: { title: "조회암호 : " },
                                editable: { type: "text", maxlength: 10, size: 14, validate: { rule: "required" } }, encrypt: true
                            }
                        ]
                    },
                    {
                        element: [
                            {
                                name: "notice", label: { title: "(초기 암호는 주민번호 뒤 7자리)"}
                            }
                        ], align: "center"
        ...
        //=====================================================================================
        var args = {
            targetid: "grdList_ZUSER", query: "DLG_ZUSER_PASSWD_CHK", title: "",
            show: false,
            element: [
    ...
    args = {
        source: {
            ...
            argument: [
                ...
                { name: "arg_passwd", value: hex_md5(gw_com_api.getValue("frmLogin", 1, "passwd")) }
            ]
        },
        target: [
            { type: "GRID", id: "grdList_ZUSER" }
        ],
...
function processRetrieveEnd(param) {
    ...
    var cnt = gw_com_api.getRowCount("grdList_ZUSER");
    if (cnt == 0) {
        ...
        var args = {
            data: {
                emp_no: gw_com_api.getValue("grdList_ZUSER", 1, "emp_no", true)
            }
        };
        processClose(args);
    }
...
function processClose(param) {
    ...
    gw_com_module.streamInterface(args);
    gw_com_api.setValue("frmLogin", 1, "passwd", "");
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        case gw_com_api.v_Stream.msg_openedDialogue:
            {
                gw_com_api.setFocus("frmLogin", 1, "passwd");
            }
            break;
