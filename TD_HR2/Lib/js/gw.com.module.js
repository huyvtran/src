...
var gw_com_module = {
    ...
    labelAssign: function (args) {
        ...
        $.each(args.row, function (i) {
            $(targetobj + "_" + this.name).text(this.value);
        });
                        ...
                        $.each(this.element, function (j) {
                            ...
                            if (this.mask != undefined) {
                                masks.push({
                                    ...
                                    readonly: false,
                                    format: "button",
                                    datepicker: (this.mask == "date-ymd") ? true : false
                                });
                            }
                        });
                        content = content +
            ...
            case "TABLE":
                {
                                        ...
                                        $.each(this.editable.child, function (j) {
                                            ...
                                            content = content +
												"<input" +
													...
													" value='" + this.value + "'" +
													" text='" + this.title + "'" +
													((this.value == evalue) ? " checked=true" : "false") +
												    " style='margin: 0px 3px 1px 0px; vertical-align: -2px;'" +
												    " class='" + estyle + "'" +
												" />";
                                            content = content +
                    ...
                    $(targetobj + " [noeditable]").dblclick(function () {
                        if (this.type == "text" || this.type == "textarea" || this.type == "html") {
                            var event = $.data($(targetobj)[0], "event");
                            if (event.itemdblclick != undefined) {
                                var param = {
                                    type: "FORM",
                ...
                $(el).change(function () {
                    if (selects[i].change != undefined) {
                        $.each(selects[i].change, function (i) {
                            ...
                            if (selects[i].data.unshift != undefined) {
                                //if (this.unshift != undefined) {
                                $.each(selects[i].data.unshift, function (j) {
                            ...
                            if (selects[i].data.push != undefined) {
                                //if (this.push != undefined) {
                                $.each(selects[i].data.push, function (j) {
            ...
            if (options.colModel.formatoptions.fix.strike != undefined) {
                ...
                if (rowObject.childNodes == undefined) {
                    ...
                    for (var col_j = 0; col_j < colmodel.length; col_j++) {
                        if (colmodel[col_j].name == options.colModel.formatoptions.fix.strike)
                        { match = col_j; break; }
                    }
    ...
    gridDownload: function (args) {
        ...
        var params =
            "?TITLE=" +
			    encodeURIComponent($("#" + args.targetid).attr("title")) +
			"&QRY_ID=" +
			    ((args.query != undefined) ? args.query : $(targetobj).attr("query")) +
            "&QRY_HDRS=" +
				((args.header == false) ? "" : this.v_Object[args.targetid].header) +
			"&QRY_COLS=" +
				this.v_Object[args.targetid].view +
            "&OPTION=" +
		        ((args.option != undefined) ? args.option : "NONE");

        var argument = $.data($("#" + args.targetid)[0], "argument");
        params = params + ((argument != undefined) ? argument : "");
        var remark = $.data($("#" + args.targetid)[0], "remark");
        params = params + ((remark != undefined) ? "&QRY_RMK=" + encodeURIComponent(remark) : "");
        $("#pageExport").attr("src", url + params);
    ...
    pageCreate: function (args) {
        ...
        var content =
			"<iframe" +
				...
				" width='" + ((args.height != undefined) ? args.height + 'px' : '100%') + "'" +
    ...
    // print. (page)
    //#region
    pagePrint: function (args) {

        switch (args.target.type) {
            case "PAGE":
            case "POPUP":
                {
                    document.frames[args.target.id + "_page"].focus();
                    document.frames[args.target.id + "_page"].processPrint();
                }
                break;
            case "TAB":
                {
                    var targetobj = args.target.id + "_" + args.target.name +
                        ((args.target.index != undefined) ? "-" + args.target.index : "");
                    document.frames[targetobj].focus();
                    document.frames[targetobj].processPrint();
                }
                break;
        }
                    ...
                    content =
			            "<iframe" +
				            ...
				            " src='" + url +
                                ...
                                "&USR_ID=" + this.v_Session.USR_ID +
                                "&USR_NM=" + this.v_Session.USR_NM +
                                "&GW_ID=" + this.v_Session.GW_ID +
                                "&EMP_NO=" + this.v_Session.EMP_NO +
                                "&DEPT_CD=" + this.v_Session.DEPT_CD +
                                "&DEPT_NM=" + this.v_Session.DEPT_NM +
                                "&POS_CD=" + this.v_Session.POS_CD +
                                "&POS_NM=" + this.v_Session.POS_NM +
					            "&DEPT_AREA=" + this.v_Session.DEPT_AREA +
					            "&DEPT_AUTH=" + this.v_Session.DEPT_AUTH +
                                "&USER_TP=" + this.v_Session.USER_TP +
					            "&DATA_TYPE=" + DataType +
				            "'" +
				            " width='100%'" +
				            " height='100%'" +
                ...
                case "GRID":
                    {
                        ...
                        var param = {
                            ...
                            select: this.select,
                            handler: this.handler,
                            handler_processed: completeRequest
                        };
                        if (args.key != undefined) {
                            var query = (this.query != undefined)
                                ? this.query : $("#" + this.id + "_data").attr("query");
                            for (var key_i = 0; key_i < args.key.length; key_i++) {
                                if (args.key[key_i].QUERY == query) {
                                    param.key = args.key[key_i].KEY;
                                    break;
                                }
                            }
                        }
                ...
                case "CHART":
                    {
                        ...
                        var param = {
                            targetid: this.id,
                            params: "&ACT=retrieve" + data.query,
                            option: args.option,
                            query: this.query,
    ...
    uploadFile: function (args) {
        ...
        switch (args.control.by) {
            case "DX":
                {
                    /*
                    var browse = $("#" + args.targetid).find(":input[id*='" + args.control.id + "_Input_0']");
                    if (browse.length == 0)
                    return;
                    $(browse).click();
                    */
    ...
    downloadFile: function (args) {
        ...
        var params =
            ...
            "&ID=" +
                encodeURIComponent(
                    gw_com_api.getValue(args.source.id, row, "file_id", true)) +
            "&PATH=" +
                encodeURIComponent(
    ...
    initPage: function (args) {
        ...
        $.blockUI();
        this.v_Session.USR_ID = gw_com_api.getPageParameter("USR_ID");
        this.v_Session.GW_ID = gw_com_api.getPageParameter("GW_ID");
        this.v_Session.USR_NM = gw_com_api.getPageParameter("USR_NM");
        this.v_Session.EMP_NO = gw_com_api.getPageParameter("EMP_NO");
        this.v_Session.DEPT_CD = gw_com_api.getPageParameter("DEPT_CD");
        this.v_Session.DEPT_NM = gw_com_api.getPageParameter("DEPT_NM");
        this.v_Session.POS_CD = gw_com_api.getPageParameter("POS_CD");
        this.v_Session.POS_NM = gw_com_api.getPageParameter("POS_NM");
        this.v_Session.DEPT_AREA = gw_com_api.getPageParameter("DEPT_AREA");
        this.v_Session.DEPT_AUTH = gw_com_api.getPageParameter("DEPT_AUTH");
        this.v_Session.USER_TP = gw_com_api.getPageParameter("USER_TP");
    ...
    streamInterface: function (args) {
        ...
        if (this.v_Current.launch == "MAIN"
			|| (args.to != undefined && args.to.type == "POPUP")) {
            if (document.frames["page_" + args.to.page] != undefined)
                document.frames["page_" + args.to.page].streamProcess(args);
            else if (this.v_Current.dialogue != null)
                document.frames["page_" + this.v_Current.dialogue].streamProcess(args);
            else if (args.from.page == "SYS_2500" && document.frames["page_w_sys2032"] != undefined)
                document.frames["page_w_sys2032"].streamProcess(args);
            else if (args.from.page == "SPC_1000" && document.frames["page_SPC_5030"] != undefined)
                document.frames["page_SPC_5030"].streamProcess(args);
            else {  // Tabpage 에서 호출한 popup Windwod의 Dialog Message 처리 : FileUpload.Save
                for (i = 0; i < document.frames.length; i++) {
                    if (document.frames[i].document.frames["page_" + args.to.page] != undefined)
                        document.frames[i].document.frames["page_" + args.to.page].streamProcess(args);
                }
            }
        }
        else {
    ...
    sendMail: function (args) {
        ...
        if (args.edit == true) {
            ...
            window.open("", "pop_email", "scrollbars=no,resizable=no,menubar=no,toolbar=no,width=1000,height=590");
        ...
        if (args.to != undefined) {
            $.each(args.to, function () {
                request.DATA.TO.push(this.name + "<" + this.value + ">");
            });
        } else if (args.mailing != undefined) {
            var data = gw_com_module.getMailingList(args.mailing);
            $.each(data, function () {
                request.DATA.TO.push(this.DATA[0] + "<" + this.DATA[1] + ">");
            });
        }
